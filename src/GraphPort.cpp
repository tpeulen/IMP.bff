/**
 *  \file GraphPort.cpp
 *  \brief The reactive value cell (see GraphPort.h).
 *
...
struct Vertex {
  void* id;
  std::shared_ptr<GraphNode> node;  // set iff the vertex is a node
  std::shared_ptr<GraphPort> port;  // set iff the vertex is a standalone port
};

Vertex vertex_of(const std::shared_ptr<GraphPort>& p) {
  std::shared_ptr<GraphNode> n = p->get_node();
  if (n) return Vertex{n.get(), n, std::shared_ptr<GraphPort>()};
  return Vertex{p.get(), std::shared_ptr<GraphNode>(), p};
}
...
const std::string& GraphObject::get_name() const { return name_; }

void GraphObject::set_name(const std::string& name) { name_ = name; }

const std::string& GraphObject::get_uid() const { return uid_; }

void GraphObject::set_uid(const std::string& uid) { uid_ = uid; }

const std::string& GraphObject::get_precursor() const { return precursor_; }

void GraphObject::set_precursor(const std::string& precursor) {
  precursor_ = precursor;
}

int GraphObject::get_death() const { return death_; }

void GraphObject::set_death(int death) { death_ = death; }

int graph_port_value_type_element(int t) {
  switch (t) {
    case GRAPH_PORT_FLOAT:
    case GRAPH_PORT_FLOAT_VECTOR: return GRAPH_PORT_FLOAT;
    case GRAPH_PORT_BOOL:
    case GRAPH_PORT_BOOL_VECTOR: return GRAPH_PORT_BOOL;
    default: return GRAPH_PORT_INT;
  }
}

bool graph_port_value_type_is_vector(int t) {
  return t == GRAPH_PORT_INT_VECTOR || t == GRAPH_PORT_FLOAT_VECTOR ||
         t == GRAPH_PORT_BOOL_VECTOR;
}

int graph_port_value_type_of(int element, bool is_vector) {
  switch (graph_port_value_type_element(element)) {
    case GRAPH_PORT_FLOAT: return is_vector ? GRAPH_PORT_FLOAT_VECTOR : GRAPH_PORT_FLOAT;
    case GRAPH_PORT_BOOL: return is_vector ? GRAPH_PORT_BOOL_VECTOR : GRAPH_PORT_BOOL;
    default: return is_vector ? GRAPH_PORT_INT_VECTOR : GRAPH_PORT_INT;
  }
}

std::string graph_port_value_type_name(int t) {
  switch (t) {
    case GRAPH_PORT_INT: return "int";
    case GRAPH_PORT_FLOAT: return "float";
    case GRAPH_PORT_INT_VECTOR: return "int[]";
    case GRAPH_PORT_FLOAT_VECTOR: return "float[]";
    case GRAPH_PORT_BOOL: return "bool";
    case GRAPH_PORT_BOOL_VECTOR: return "bool[]";
    default: return "unknown(" + std::to_string(t) + ")";
  }
}

GraphPort::GraphPort()
    : GraphObject(), buffer_(1, 0.0), value_type_(GRAPH_PORT_FLOAT),
      is_vector_(false) {
...
GraphPort::GraphPort(double value, bool fixed, bool is_output, bool is_reactive,
...
GraphPort::GraphPort(long long value, bool fixed, bool is_output, bool is_reactive,
           bool is_bounded, double lb, double ub, int value_type,
           const std::string& name)
    : GraphObject(name),
...
GraphPort::GraphPort(bool value, bool fixed, bool is_output, bool is_reactive,
           bool is_bounded, double lb, double ub, int value_type,
           const std::string& name)
    : GraphObject(name),
  ...
  if (graph_port_value_type_element(value_type_) != GRAPH_PORT_BOOL) {
    value_type_ = graph_port_value_type_of(GRAPH_PORT_BOOL, false);
  }
  finalize_storage();
}

GraphPort::GraphPort(const std::vector<double>& values, bool fixed, bool is_output,
...
GraphPort::~GraphPort() = default;

double GraphPort::get_value() const {
  const GraphPort* src = link_ ? link_.get() : this;
...
std::vector<double> GraphPort::get_value_vector() const {
  return get_values_ref();
}

const std::vector<double>& GraphPort::get_values_ref() const {
  const GraphPort* src = link_ ? link_.get() : this;
  ...
  if (graph_port_value_type_element(src->value_type_) == GRAPH_PORT_FLOAT) {
    return src->buffer_;
  }
...
void GraphPort::get_value_view(double** out_values, int* n_out_values) const {
...
long long GraphPort::get_value_int() const {
  const GraphPort* src = link_ ? link_.get() : this;
...
std::vector<long long> GraphPort::get_value_vector_int() const {
  const GraphPort* src = link_ ? link_.get() : this;
  const bool as_float =
      graph_port_value_type_element(src->value_type_) == GRAPH_PORT_FLOAT;
...
void GraphPort::set_value_bool(bool v) {
  ...
  // set_value_type()'s -- see GraphPortValueType.
  write_value(v ? 1.0 : 0.0, false);
}

bool GraphPort::get_value_bool() const {
  const GraphPort* src = link_ ? link_.get() : this;
  return !src->buffer_.empty() && src->element_as_double(0) != 0.0;
}

std::vector<int> GraphPort::get_value_vector_bool() const {
  const GraphPort* src = link_ ? link_.get() : this;
  std::vector<int> out;
  out.reserve(src->buffer_.size());
  for (std::size_t i = 0; i < src->buffer_.size(); ++i) {
...
void GraphPort::set_value_vector_int(const std::vector<long long>& v) {
  ...
  value_type_ = GRAPH_PORT_INT_VECTOR;
  is_vector_ = true;
  store_ints(v);
  finalize_storage();
  update_attached_node();
...
void GraphPort::set_value_vector(const std::vector<double>& v) {
  write_vector(v, /*input_is_float=*/true);
}

void GraphPort::set_values_array(double* in_values, int n_values) {
...
long long GraphPort::slot_as_int(double slot) {
  long long v;
  std::memcpy(&v, &slot, sizeof(v));
  return v;
}

double GraphPort::int_as_slot(long long v) {
  double slot;
  std::memcpy(&slot, &v, sizeof(slot));
  return slot;
}

double GraphPort::element_as_double(std::size_t i) const {
  if (graph_port_value_type_element(value_type_) == GRAPH_PORT_FLOAT) return buffer_[i];
  return static_cast<double>(slot_as_int(buffer_[i]));
}

void GraphPort::store_doubles(const std::vector<double>& v) {
  const int element = graph_port_value_type_element(value_type_);
  if (element == GRAPH_PORT_FLOAT) {
    ...
    for (std::size_t i = 0; i < v.size(); ++i) {
      ...
      const long long as_int =
          (element == GRAPH_PORT_BOOL) ? ((e != 0.0 && !std::isnan(e)) ? 1 : 0)
...
void GraphPort::store_ints(const std::vector<long long>& v) {
  ...
  for (std::size_t i = 0; i < v.size(); ++i) {
    if (element == GRAPH_PORT_FLOAT) {
      ...
      buffer_[i] = int_as_slot((element == GRAPH_PORT_BOOL) ? (v[i] != 0 ? 1 : 0)
                                                      : v[i]);
    }
  }
  double_cache_valid_ = false;
...
void GraphPort::finalize_storage() {
  if (is_bounded_) clip_to_bounds();
  double_cache_valid_ = false;
}

bool GraphPort::is_bool_typed() const {
  return graph_port_value_type_element(value_type_) == GRAPH_PORT_BOOL;
}
...
void GraphPort::copy_from_link(const GraphPort& source) {
  ...
  if (!source.get_is_vector() && source.current_size() == 1) {
    ...
    value_type_ = graph_port_value_type_of(element, true);
    is_vector_ = true;
...
void GraphPort::write_vector(const std::vector<double>& v, bool input_is_float) {
  ...
  if (input_is_float && sanitize_ && element == GRAPH_PORT_FLOAT) {
    for (double& e : incoming) e = sanitize(e);
  }
  value_type_ = graph_port_value_type_of(element, true);
  is_vector_ = true;
...
int GraphPort::get_value_type() const { return value_type_; }

void GraphPort::set_value_type(int t) {
  ...
  const bool was_float = graph_port_value_type_element(value_type_) == GRAPH_PORT_FLOAT;
...
bool GraphPort::get_is_vector() const { return is_vector_; }

unsigned int GraphPort::current_size() const {
  return static_cast<unsigned int>(buffer_.size());
}

bool GraphPort::is_valid() const { return true; }

bool GraphPort::get_fixed() const { return fixed_; }

void GraphPort::set_fixed(bool v) { fixed_ = v; }

bool GraphPort::get_is_output() const { return is_output_; }

void GraphPort::set_port_type(bool v) { is_output_ = v; }

bool GraphPort::get_is_reactive() const { return is_reactive_; }

void GraphPort::set_is_reactive(bool v) { is_reactive_ = v; }

bool GraphPort::get_is_bounded() const { return is_bounded_; }

void GraphPort::set_is_bounded(bool v) {
  is_bounded_ = v;
  if (is_bounded_) clip_to_bounds();
}

double GraphPort::get_lower_bound() const { return lb_; }

double GraphPort::get_upper_bound() const { return ub_; }

void GraphPort::set_bounds(double lb, double ub) {
  lb_ = lb;
  ub_ = ub;
  if (is_bounded_) clip_to_bounds();
}

void GraphPort::clip_to_bounds() {
  const int element = graph_port_value_type_element(value_type_);
  for (std::size_t i = 0; i < buffer_.size(); ++i) {
...
const std::string& GraphPort::get_prior() const { return prior_; }

void GraphPort::set_prior(const std::string& json) { prior_ = json; }

std::shared_ptr<GraphPort> GraphPort::get_link() const { return link_; }

void GraphPort::set_link(std::shared_ptr<GraphPort> v) {
  if (v.get() == link_.get()) return;
  if (v && would_create_cycle(v)) {
    throw GraphLinkCycleError(
        "Linking these ports would create a cycle; "
        "the port graph must remain acyclic.");
  }
  unlink();
  if (!v) return;
  std::weak_ptr<GraphPort> self;
  try {
    self = shared_from_this();
  } catch (const std::bad_weak_ptr&) {
    throw std::runtime_error(
        "a GraphPort that follows another GraphPort must be owned by a "
  ...
  if (link_) {
    ...
    followers.erase(
        std::remove_if(followers.begin(), followers.end(),
                       [this](const std::weak_ptr<GraphPort>& w) {
                         std::shared_ptr<GraphPort> p = w.lock();
                         return !p || p.get() == this;
                       }),
        followers.end());
    link_.reset();
...
bool GraphPort::is_linked() const { return link_ != nullptr; }

bool GraphPort::would_create_cycle(const std::shared_ptr<GraphPort>& v) {
  if (!v) return false;

  std::shared_ptr<GraphPort> self;
  try {
    self = shared_from_this();
  } catch (const std::bad_weak_ptr&) {
    throw std::runtime_error(
        "a GraphPort whose links are being checked must be owned by a "
        "std::shared_ptr (every Python-wrapped port is)");
  }
  ...
  for (const Vertex& n : nodes) {
    if (n.node) {
      for (const auto& kv : n.node->get_input_ports()) {
        const GraphPort* p = kv.second.get();
...
std::shared_ptr<GraphNode> GraphPort::get_node() const { return node_.lock(); }

void GraphPort::set_node(std::shared_ptr<GraphNode> n) { node_ = n; }
...
void GraphPort::touch_attached_node_structure() {
  if (std::shared_ptr<GraphNode> n = node_.lock()) n->touch_structure();
}

void GraphPort::update_attached_node() {
  if (std::shared_ptr<GraphNode> n = node_.lock()) {
    ...
    // makes the comparison possible. See GraphNode::invalidate_from_port.
...
void GraphPort::propagate_to_followers() {
...
std::string GraphPort::describe() const {
  std::ostringstream out;
  out << "GraphPort(name='" << get_name() << "', uid='" << get_uid()
