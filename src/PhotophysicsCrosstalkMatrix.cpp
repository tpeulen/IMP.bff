/**
 *  \file IMP/bff/PhotophysicsCrosstalkMatrix.cpp
...
// --------------------------------------------------------- PhotophysicsCrosstalkMatrix

PhotophysicsCrosstalkMatrix::PhotophysicsCrosstalkMatrix() {}

PhotophysicsCrosstalkMatrix::PhotophysicsCrosstalkMatrix(const std::vector<std::string>& rows,
  ...
  if (values.size() != expected) {
    throw std::invalid_argument(
        "PhotophysicsCrosstalkMatrix: got " + std::to_string(values.size()) +
...
int PhotophysicsCrosstalkMatrix::index_of(const std::vector<std::string>& labels,
...
double PhotophysicsCrosstalkMatrix::get(int row, int column) const {
  if (row < 0 || row >= get_n_rows() || column < 0 ||
      column >= get_n_columns()) {
    throw std::out_of_range("PhotophysicsCrosstalkMatrix::get: index out of range");
  }
  return values_[static_cast<std::size_t>(row) * columns_.size() + column];
}

double PhotophysicsCrosstalkMatrix::get(const std::string& row,
...
PhotophysicsCrosstalkMatrix PhotophysicsCrosstalkMatrix::select(
  ...
  return PhotophysicsCrosstalkMatrix(rows, columns, out);
}

void PhotophysicsCrosstalkMatrix::scale_row(const std::string& label, double factor) {
  const int i = index_of(rows_, label);
  if (i < 0) {
    throw std::out_of_range("PhotophysicsCrosstalkMatrix::scale_row: no row " + label);
  }
...
void PhotophysicsCrosstalkMatrix::scale_column(const std::string& label, double factor) {
  const int j = index_of(columns_, label);
  if (j < 0) {
    throw std::out_of_range("PhotophysicsCrosstalkMatrix::scale_column: no column " +
                            label);
  }
