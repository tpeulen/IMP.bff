...
RotamerLibrary normalize_weights(const RotamerLibrary& lib) {
    RotamerLibrary out = lib;
...
MfdbTags drot_provenance(const std::string& path, const std::string& library) {
    MfdbTags out;
    if (!PtoReader::looks_like_pto(path)) return out;   // pre-PTO: not stated
    const PtoReader pto(path);
...
void add_object(PtoWriter& pto, const std::string& name,
...
void write_drot_bundle(const std::vector<std::string>& sources,
    ...
    cj << "]}";
    const DrotEncoding defaults;
    add_object(pto, "drot.catalog", "drot.catalog", "json+brotli",
               to_bytes(cj.str()), defaults);

    for (std::size_t i = 0; i < sources.size(); ++i) {
        const PtoReader in(sources[i]);
        const std::vector<PtoObject>& objects = in.objects();
        if (objects.empty()) {
...
void write_drot(const std::string& path,
                double* rotamer_coords, int n_rotamer_coords,
                const std::vector<std::string>& atom_names,
                const std::vector<std::string>& elements,
                const std::vector<std::string>& resnames,
...
void write_drot_with_provenance(const std::string& path,
                double* rotamer_coords, int n_rotamer_coords,
                const std::vector<std::string>& atom_names,
                const std::vector<std::string>& elements,
                const std::vector<std::string>& resnames,
...
RotamerLibrary read_rotamer_library(const std::string& path) {
...
void DunbrackRotamers::get_chi(double** out_view, int* n_out_view) const {
    internal::copy_to_view(chi, out_view, n_out_view);
}
void DunbrackRotamers::get_sigma(double** out_view, int* n_out_view) const {
    internal::copy_to_view(sigma, out_view, n_out_view);
}
void DunbrackRotamers::get_probability(double** out_view,
                                       int* n_out_view) const {
    internal::copy_to_view(probability, out_view, n_out_view);
}

void write_dunbrack_library(const std::string& bin_path,
                            const std::string& path) {
    ...
    pto.add("bbdep.json", "rot.bbdep.header", "json+brotli",
            hp.empty() ? NULL : &hp[0], hp.size());
    ...
    pto.add("drot.catalog", "drot.catalog", "json+brotli",
            cp.empty() ? NULL : &cp[0], cp.size());
    ...
    for (std::size_t i = 0; i < kResidueCount; ++i) {
        ...
        pto.add(object_name(r.code), "rot.bbdep.records", "faspr20+brotli",
                packed.empty() ? NULL : &packed[0], packed.size());
    }
    std::fclose(f);
    pto.close();
...
void write_dunbrack_bin(const std::string& path, const std::string& bin_path) {
    const PtoReader pto(path);
    ...
    for (std::size_t i = 0; i < kResidueCount; ++i) {
        const Residue& r = kResidues[i];
        const int at = pto.find(object_name(r.code));
        if (at < 0) {
            std::fclose(out);
...
DunbrackRotamers read_dunbrack_rotamers(const std::string& path, char residue_,
