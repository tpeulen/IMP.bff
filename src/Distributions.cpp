...
std::vector<double> normal_density(const std::vector<double>& x, double loc,
                                   double scale) {
    return internal::normal_density_t<double>(x, loc, scale);
}
...
std::vector<double> generalized_normal_density_impl(
        ...
        bool norm) {
    return internal::generalized_normal_density_t<double>(x, loc, scale, shape, norm);
}
...
std::vector<double> distance_between_gaussian_impl(
    ...
    // for every non-positive separation, and the separated branch divides by
    // it. The arithmetic is internal/DistanceKernels.h's, shared with the
    // derivatives the distance nodes report.
    return internal::distance_between_gaussian_t<double>(distances, separation_distance,
                                                         sigma);
}
