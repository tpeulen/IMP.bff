...
The landing page at ``doc/static_root/index.html`` fetches the resulting
``versions.json`` and renders one link per published documentation version.
Called from the ``build_docs`` job of ``.github/workflows/ci.yml``.

Same contract as tttrlib's ``tools/update_docs_versions.py``.
"""
...
def load_versions(path: Path) -> list:
    if not path.exists():
        return []
    try:
