"""The chinet-shaped keyword constructor on bff.GraphPort.

chisurf's Parameter builds its ports through chinet's Python constructor
surface -- ``GraphPort(value=..., name=..., lb=..., ub=..., is_bounded=...)`` --
so phase 3 of removing chinet needs the same call to land on
``IMP.bff.GraphPort`` unchanged. SWIG cannot generate keyword arguments for an
...
def test_kwargs_ctor_float_scalar():
    p = bff.GraphPort(value=3.5, name="amp")
    assert p.value == 3.5
    assert p.name == "amp"
...
def test_kwargs_ctor_int_scalar_stays_int():
    p = bff.GraphPort(value=5)
    assert p.value == 5
    assert p.get_value_type() == 0


def test_kwargs_ctor_numpy_scalars():
    p = bff.GraphPort(value=np.float64(2.5))
    assert p.value == 2.5
    assert p.get_value_type() == 1

    q = bff.GraphPort(value=np.int64(7))
    assert q.value == 7
...
def test_kwargs_ctor_chisurf_shape():
    ...
    p = bff.GraphPort(value=np.atleast_1d(3.5).astype(np.float64),
...
def test_kwargs_ctor_bounds_clip_initial_value():
    p = bff.GraphPort(value=np.array([50.0]), lb=0.0, ub=10.0, is_bounded=True)
    assert np.allclose(p.value, [10.0])


def test_kwargs_ctor_flags():
    p = bff.GraphPort(value=1.0, fixed=True, is_output=True, is_reactive=True)
...
def test_kwargs_ctor_prior_dict():
    p = bff.GraphPort(value=1.0, prior={"kind": "normal", "mu": 1.0,
                                   "sigma": 0.5})
...
def test_link_property_is_assignable():
    master = bff.GraphPort(value=7.0)
    follower = bff.GraphPort(value=0.0)
...
def test_get_json_read_json_round_trip():
    p = bff.GraphPort(value=np.array([2.0]), name="x", lb=-1.0, ub=5.0,
                 is_bounded=True, fixed=True,
                 prior={"kind": "lognormal", "mu": 0.0, "sigma": 0.4})
    doc = p.get_json()
    q = bff.GraphPort(value=0.0)
...
def test_read_json_restores_value_of_fixed_port():
    ...
    p = bff.GraphPort(value=9.0, fixed=True)
    q = bff.GraphPort(value=0.0, fixed=True)
    q.read_json(p.get_json())
    assert q.value == 9.0
    assert q.fixed is True
