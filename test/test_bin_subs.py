"""The compiled command line: ``IMP.bff.command_line_main`` and its subs.

The bin/ scripts whose subject is the core became C++ subcommands
(src/CommandLine.cpp), so they ship with the wheel and run with no IMP. These tests
...
def _write_library(tmp_path, xyz, name):
    ...
    IMP.bff.write_probe_rotamer_drot(str(path), numpy.ascontiguousarray(xyz).reshape(-1),
...
def test_help_lists_the_subs(capfd):
    ...
    assert IMP.bff.command_line_main(["help"]) == 0
...
def test_unknown_sub_is_a_usage_error():
    assert IMP.bff.command_line_main(["no-such-sub"]) != 0
...
def test_pdb2cif_writes_an_atom_site_table(tmp_path, ensemble):
    ...
    assert IMP.bff.command_line_main(["pdb2cif", str(pdb), str(cif),
                             "--probe-id", "LYP"]) == 0
    text = cif.read_text()
...
def test_traj2drot_round_trips_through_the_container(tmp_path, ensemble):
    ...
    assert IMP.bff.command_line_main(["traj2drot", str(src), str(dst)]) == 0
...
def test_traj2drot_bundle_reads_back_by_library_name(tmp_path, ensemble):
    ...
    assert IMP.bff.probe_rotamer_drot_catalog(str(family)) == ("one", "two")
    assert IMP.bff.read_probe_rotamer_drot(str(family), "two").n_rotamers == 7
