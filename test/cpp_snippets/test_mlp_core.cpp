// bff's differentiable MLP core (include/internal/MlpCore.h), the Dual.h
// operators it needs, the MatGemm policy (internal/MlpGemm.h) and the int8
// path (internal/MlpQuant.h). Ported from tttrlib's test/cpp/test_mlp_core.cpp
// (removed there with the network in 10858cf19); sections 9-10 are new.
//
// Built as a module test by IMP's CMake (no arguments: the fixtures are found
// next to this file, ../input/nn), and compiled standalone by
// test/test_neural_net_derivatives.py against bff's headers only:
//
//   c++ -std=c++17 -O2 -I <dir holding IMP/bff/internal> \
//       test/cpp_snippets/test_mlp_core.cpp -o /tmp/test_mlp_core
//   /tmp/test_mlp_core test/input/nn
//
...
namespace mq = IMP::bff::internal::mlpquant;
...
static void test_activation_derivatives() {
    ...
    for (Activation a : acts) {
        ...
        char buf[64];
        std::snprintf(buf, sizeof buf, "%s f'", IMP::bff::internal::activation_to_string(a).c_str());
...
static void test_forward_taylor(Activation hidden) {
    std::printf("forward Taylor orders, hidden=%s\n", IMP::bff::internal::activation_to_string(hidden).c_str());
    Lcg rng(21);
...
static void test_model_scalers() {
    ...
    auto loss = [&](const IMP::bff::internal::MlpModel& mm, const std::vector<double>& x, const std::vector<double>& v) {
    ...
    {
        IMP::bff::internal::StandardScaler sc; sc.fit(X.data(), n, n_in);
        double worst = 0;
        for (int j = 0; j < n_in; ++j) {
        ...
        bool threw = false;
        IMP::bff::internal::MlpModel bad = m; bad.x_scaler.mean.push_back(0.0); bad.x_scaler.scale.push_back(1.0);
...
static void test_onnx_import(const std::string& fixture_dir) {
    ...
    for (const Case& c : cases) {
        ...
        try {
            IMP::bff::internal::MlpModel m = mc::model_from_onnx(bytes);
            std::vector<double> y, d1, d2;
...
// --------------------------------------------------------------------------
// 9. MatGemm (Mat.h's blocked kernels) vs PortableGemm: same arithmetic up to
//    reassociation, forward (orders 0-2) and backward, <= 1e-12 relative
// --------------------------------------------------------------------------
static double max_rel_diff(const std::vector<double>& a, const std::vector<double>& b) {
    double scale = 0.0, m = 0.0;
    for (size_t i = 0; i < a.size(); ++i) {
        scale = std::max(scale, std::abs(b[i]));
        m = std::max(m, std::abs(a[i] - b[i]));
    }
    return scale > 0.0 ? m / scale : m;
}
...
static IMP::bff::internal::MlpModel make_model(const std::vector<int>& dims, Activation hidden,
                                               Lcg& rng) {
    std::vector<Activation> acts(dims.size() - 1, hidden);
    acts.back() = Activation::Identity;
    IMP::bff::internal::MlpModel m;
    m.layers = make_net(dims, acts, rng);
    // active scalers, so the chain rule through them is part of the parity
    m.x_scaler.mean.assign(static_cast<size_t>(dims.front()), 0.1);
    m.x_scaler.scale.assign(static_cast<size_t>(dims.front()), 1.7);
    m.y_scaler.mean.assign(static_cast<size_t>(dims.back()), -0.3);
    m.y_scaler.scale.assign(static_cast<size_t>(dims.back()), 2.5);
    return m;
}

static void test_matgemm_parity() {
    std::printf("MatGemm vs PortableGemm (forward orders 0-2, backward)\n");
    const int batches[] = {1, 3, 17, 256};
    for (int n : batches) {
        Lcg rng(101 + n);
        const int n_in = 5, n_out = 3;
        auto m = make_model({n_in, 64, 64, 64, n_out}, Activation::Tanh, rng);
        std::vector<double> X(static_cast<size_t>(n) * n_in), V(X.size()), W(static_cast<size_t>(n) * n_out);
        for (auto& v : X) v = 2.0 * rng.uniform();
        for (auto& v : V) v = rng.uniform();
        for (auto& v : W) v = rng.uniform();
        std::vector<double> ya, d1a, d2a, yb, d1b, d2b;
        mc::model_predict<mc::PortableGemm>(m, X.data(), n, 2, V.data(), ya, d1a, d2a);
        mc::model_predict<MatGemm>(m, X.data(), n, 2, V.data(), yb, d1b, d2b);
        const double fwd = std::max(max_rel_diff(yb, ya),
                                    std::max(max_rel_diff(d1b, d1a), max_rel_diff(d2b, d2a)));
        char buf[96];
        std::snprintf(buf, sizeof buf, "forward, batch %d", n);
        report(fwd <= 1e-12, buf, fwd, 1e-12);
        std::vector<double> pa, xa, va, pb, xb, vb;
        mc::model_backward<mc::PortableGemm>(m, X.data(), n, V.data(), W.data(), W.data(), W.data(), pa, xa, va);
        mc::model_backward<MatGemm>(m, X.data(), n, V.data(), W.data(), W.data(), W.data(), pb, xb, vb);
        const double bwd = std::max(max_rel_diff(pb, pa),
                                    std::max(max_rel_diff(xb, xa), max_rel_diff(vb, va)));
        std::snprintf(buf, sizeof buf, "backward (order 2 adjoint), batch %d", n);
        report(bwd <= 1e-12, buf, bwd, 1e-12);
    }
    // Timing, reported only: forward and an order-0 backward, 3x64 tanh net.
    for (int n : {256, 512}) {
        Lcg rng(7);
        auto m = make_model({8, 64, 64, 64, 4}, Activation::Tanh, rng);
        std::vector<double> X(static_cast<size_t>(n) * 8), W(static_cast<size_t>(n) * 4, 1.0), y, a, b, p, dx, dv;
        for (auto& v : X) v = rng.uniform();
        const double tp = best_seconds(20, [&] { mc::model_predict<mc::PortableGemm>(m, X.data(), n, 0, nullptr, y, a, b); });
        const double tm = best_seconds(20, [&] { mc::model_predict<MatGemm>(m, X.data(), n, 0, nullptr, y, a, b); });
        const double bp = best_seconds(20, [&] { mc::model_backward<mc::PortableGemm>(m, X.data(), n, nullptr, W.data(), nullptr, nullptr, p, dx, dv); });
        const double bm = best_seconds(20, [&] { mc::model_backward<MatGemm>(m, X.data(), n, nullptr, W.data(), nullptr, nullptr, p, dx, dv); });
        std::printf("  time  batch %d, 3x64: forward portable %.1f us, MatGemm %.1f us (%.2fx); "
                    "backward portable %.1f us, MatGemm %.1f us (%.2fx)\n",
                    n, tp * 1e6, tm * 1e6, tp / tm, bp * 1e6, bm * 1e6, bp / bm);
    }
}

// --------------------------------------------------------------------------
// 10. int8 dynamic-range inference (MlpQuant.h, rebuilt from its spec)
// --------------------------------------------------------------------------
static void test_quant() {
    std::printf("int8 inference (MlpQuant.h)\n");
    Lcg rng(33);
    auto m = make_model({8, 64, 64, 64, 4}, Activation::Tanh, rng);
    const mq::QuantModel q = mq::quantize(m);
    size_t n_w = 0;
    for (const auto& l : m.layers) n_w += l.weight.size();
    report(q.weight_bytes() * 8 == n_w * sizeof(double), "weights 8x smaller than double",
           double(n_w * sizeof(double)) / double(q.weight_bytes()), 8.0);
    const int n = 256;
    std::vector<double> X(static_cast<size_t>(n) * 8);
    for (auto& v : X) v = 2.0 * rng.uniform();
    std::vector<double> y, a, b, yq;
    mc::model_predict<mc::PortableGemm>(m, X.data(), n, 0, nullptr, y, a, b);
    mq::predict(q, X.data(), n, yq);
    // An untrained net with O(1) weights on 64 inputs is the harsh case (wide
    // pre-activations); the 2 % of the spec is pinned on the fixtures and a
    // trained net in test_neural_net_derivatives.py. Measured here: ~2.4 %.
    const double err = max_rel_diff(yq, y);
    report(err <= 0.05, "error <= 5% of the output absmax (random 3x64 net)", err, 0.05);
    // a row's result does not depend on the rest of the batch
    std::vector<double> y1;
    mq::predict(q, X.data() + 8 * 5, 1, y1);
    report(max_abs_diff(y1, std::vector<double>(yq.begin() + 4 * 5, yq.begin() + 4 * 6)) == 0.0,
           "per-row requantisation: batch-independent", 0.0, 0.0);
    const double td = best_seconds(20, [&] { mc::model_predict<mc::PortableGemm>(m, X.data(), n, 0, nullptr, y, a, b); });
    const double tq = best_seconds(20, [&] { mq::predict(q, X.data(), n, yq); });
    std::printf("  time  batch %d, 3x64: double portable %.1f us, int8 %.1f us (%.2fx)\n",
                n, td * 1e6, tq * 1e6, td / tq);
}
...
int main(int argc, char** argv) {
    ...
    test_matgemm_parity();
    test_quant();
    // fixture dir: argv[1], else ../input/nn next to this source file
    std::string here(__FILE__);
    here = here.substr(0, here.find_last_of("/\\") + 1);
    test_onnx_import(argc > 1 ? argv[1] : here + "../input/nn");
