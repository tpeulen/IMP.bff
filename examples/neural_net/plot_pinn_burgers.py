...
in about a minute in pure NumPy + IMP.bff: ``ν = 0.05``, a 2-20-20-20-1 network,
...
def objective(p):
    ...
    u, u_x, u_xx = net.predict_derivatives(X_f, e_x, order=2)
    ...
    w = 2 * r / r.size
    g_x, _, _ = net.backward_derivatives(X_f, e_x, w * u_x, dY1=w * u, dY2=-nu * w)
    g_t, _, _ = net.backward_derivatives(X_f, e_t, np.zeros_like(r), dY1=w)
...
u_nn = predict(net, XT).reshape(xg.shape)
u_ex = burgers_exact(xg[::5, ::5], tg[::5, ::5], nu)     # coarser: the quadrature is a Python loop
n_pts = XT.shape[0]
u, u_x, u_xx = net.predict_derivatives(XT, np.tile([[1.0, 0.0]], (n_pts, 1)), order=2)
_, u_t, _ = net.predict_derivatives(XT, np.tile([[0.0, 1.0]], (n_pts, 1)), order=1)
