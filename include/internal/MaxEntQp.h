...
struct MaxEntResult {
    ...
    int niter = 0;
    /*!
     * \brief The solve ran to completion without failing.
     *
     * **Not** "it converged": a solve that exhausts `max_iter` with the
     * stopping test still unmet is a completed solve, and this stays true for
     * it. That is what it has always meant, and callers reading it are left
     * alone; `converged` below is the question most of them were asking.
     */
    bool success = false;
    /*!
     * \brief The stopping test was actually met: `dgrad <= tol` at exit.
     *
     * False when the loop ran out of iterations instead. The two are different
     * facts and were reported as one until 2026-09-09, so an L-curve scan over
     * `nu` -- which is how `nu` gets chosen -- silently kept unconverged
     * points, and the symptom is a distribution that looks smooth because the
     * solver stopped rather than because the data asked for it.
     */
    bool converged = false;
    /*!
     * \brief The Skilling-Bryan gradient angle at exit, the quantity `tol` is
     *        compared against.
     *
     * Half the norm of the difference between the normalised chi-square and
     * entropy gradients. At the constrained optimum the two are antiparallel
     * and this goes to zero. Reported so a caller can see *how far* from
     * converged an unconverged solve was, rather than only that it was.
     */
    double dgrad = 0.0;
};
