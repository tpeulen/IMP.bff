/**
 * \file IMP/bff/SamplerDiagnostics.h
 ...
 *
 * Header-only. Chains are `std::vector<std::vector<double>>`, one row per chain, all of equal
 * length; chains must be independent (an ensemble's walkers are not -- see Sampling.h).
 * Written 2026-09-15 for PRD-145 (checked against arviz, test/decay/test_mcmc_diagnostics.py);
 * moved here from tttrlib, where nothing used it, by PRD-147 (tpeulen: samplers belong in bff).
 */
#ifndef IMPBFF_SAMPLERDIAGNOSTICS_H
#define IMPBFF_SAMPLERDIAGNOSTICS_H
