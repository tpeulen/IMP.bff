/**
 *  \file IMP/bff/PhotophysicsCrosstalkMatrix.h
 ...
 */
#ifndef IMPBFF_PHOTOPHYSICSCROSSTALKMATRIX_H
#define IMPBFF_PHOTOPHYSICSCROSSTALKMATRIX_H
...
/**
    ...
    label with zeros.
 */
class IMPBFFEXPORT PhotophysicsCrosstalkMatrix {
 public:
  //! The empty matrix.
  PhotophysicsCrosstalkMatrix();

  //! ``n_rows x n_columns`` values, row-major; throws on a size mismatch.
  PhotophysicsCrosstalkMatrix(const std::vector<std::string>& rows,
   ...
   */
  PhotophysicsCrosstalkMatrix select(const std::vector<std::string>& rows,
...
#endif /* IMPBFF_PHOTOPHYSICSCROSSTALKMATRIX_H */
