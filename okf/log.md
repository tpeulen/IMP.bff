...
- **Ternary networks: BitNet b1.58 inference and quantisation-aware training** ([neural-net.md](neural-net.md), "Ternary").
  - `QuantizedNeuralNet(net, "ternary" | "ternary_row" | "ternary_tq1" | "ternary_tq1_row")`: absmean ternary weights (per tensor; `_row` per row for PTQ), int8 absmax activations per row (W1.58A8), ties to even; stored 2 bits a weight (TQ2_0-style codes `q + 1`) or five trits a byte (ggml's TQ1_0 base-3 byte, 1.6 bits); `bff.quantized_neural_net` round trip bit-exact. `internal/MlpTernary.h`: ternary x int8 kernels for NEON dotprod, NEON, AVX2 (`maddubs`), AVX-512 VNNI (`dpbusd`) and generic, bit-identical (fingerprint `4f6b5c542ab55f36` on 4 Mac and 4 cordeshub builds). Sources credited with MIT notices (microsoft/unilm BitNet, microsoft/BitNet, ggml).
  - `NeuralNetTrainOptions.precision = "ternary"` (`internal/MlpTernaryTrain.h`): float64 master weights and Adam, fprop on the ternary kernel, straight-through estimator for both quantisers (`dW = dZ^T Â`, `dA = dZ Ŵ`, no gradient into the scales), float64 dgrad/wgrad, first/last layer float64 (options); `get_quantized_network()` predicts training's forward pass bit for bit.
  - Accuracy: 4-64-64-64-2 regression held-out MSE float64 8.45e-4, ternary QAT 1.52e-3 (1.80x; every layer ternary 2.20x), ternary PTQ 0.233 (276x); HMM surrogate MAE float64 0.0959, QAT 0.0939, PTQ 0.173. Sizes on 24-256-256-128-8: 2.002 / 1.630 bits a weight.
  - Speed: inference 24-256-256-128-8 ternary / float64 at batch 1/32/256 -- M1 0.30/0.48/0.58 (Python), Xeon 4416+ AVX2 0.28/0.29/0.48, AVX-512 0.31/0.25/0.39 (C++, load 15-17). Training an epoch (M1): surrogate 0.82x, 64-wide 0.92x, tiny 1.03x float64 -- only fprop is integer by the recipe. Tests: `cpp_snippets/test_ternary_kernels.cpp` (pytest matrix, CI `fp4_kernels_x86`), `test_neural_net_ternary.py`; `bench_neural_net_fp4.py --ternary`.

- **A vectorised tanh for every network path (`internal/MlpMath.h`)** ([neural-net.md](neural-net.md), "Fast tanh"). By decision, float64 results through tanh / sigmoid / SiLU change by rounding.
  - Cephes' rational form below 0.625, `1 - 2 / (exp(2|x|) + 1)` above, and a Cody-Waite / Estrin exp; NEON, SSE2 (the x86-64 baseline), AVX2, AVX-512 and generic variants, all bit-identical (one fingerprint on 5 Mac and 5 cordeshub builds). MlpCore's `act_apply`, `act_value<double>` and the tanh / sigmoid derivatives use it, so int8 / FP4 inference, FP4 training and the float64 paths share it. Accuracy: tanh within 1 ulp of Apple's libm and of the true value (x86 `tanhl`), within 2 ulp of glibc 2.35; sigmoid / SiLU within 2 ulp; odd, monotone, NaN / inf / -0 exact. PyTorch fixtures 1.7e-16 (double) and 6.3e-8 (float32); no tolerance changed. The FP4 training fingerprint now includes a tanh net: `64407c22cd039ecf`.
  - tanh per element: M1 3.3 ns (libm 8.3), Xeon AVX-512 2.6 / AVX2 3.9 / SSE2 7.4 ns (glibc 19.7). Gates: x86 batch-256 inference float64 5.9 -> 2.3-2.5 ms, FP4 5.1 -> 1.0-1.7 ms; M1 2.3 -> 2.0 ms and 1.47 -> 1.17 ms. The 64-wide training step got faster on both paths (x86 float64 2.1x, M1 1.15x), but FP4 / float64 moved toward the arithmetic ratio instead of below it: 0.87-0.92 a step and 0.80-0.91 an epoch on the Mac, 0.89-1.18 on x86. The 0.8 goal is not met, because tanh is shared by both paths. Test: `cpp_snippets/test_mlp_math.cpp` (pytest matrix and CI `fp4_kernels_x86`).
...
- **FP4 at float64 speed or better: blocked micro-kernels, vectorised quantisation, AVX-512 VNNI** ([neural-net.md](neural-net.md), "Performance design" and "Speed gates").
  - `MlpFp4Kernels.h`: register-blocked FP4 GEMM on weights repacked once (NEON dotprod `vdotq_laneq_s32` 4 x 4, plain NEON, AVX2 `maddubs` 2 x 8, new **AVX-512 VNNI** `dpbusd` 4 x 16 with ggml's +128 offset trick); quantisers (absmax, E2M1 incl. stochastic rounding with the unchanged SplitMix64 stream, int8, NEON round-to-odd narrowing, exact reciprocal for power-of-two scales, bit-level E4M3); `QuantizedNeuralNet` packs once. `MlpFp4Train.h`: weights quantised once a step into both W and W^T operands, Hadamard fused with the transpose.
  - Numerics unchanged: golden hashes (quantisers, GEMMs, predict, FP4 training steps) equal to the first FP4 commit on the Mac's four variants and cordeshub's four (generic, AVX2, AVX-512, `-march=sapphirerapids`), and the Python-trained networks' bytes are identical. Every variant is now bit-identical to generic under any flags (before, g++ `-mfma` contracted the float combine).
  - G1 (inference, 24-256-256-128-8/-15, batch 1/32/256): every FP4 format and W4A8/W4A4 faster than float64, worst cell over batch 1/32/256 -- M1 0.33/0.62/0.71x, Xeon 4416+ AVX2 0.49/0.67/0.91x, AVX-512 0.47/0.73/0.82x (bff has no float32 path). G2 (training, ms an epoch, all recipe features): surrogate 256-256-128 nvfp4 0.75x, mxfp4 0.74x float64 (was 1.8x / 1.5x); 64-wide regression 1.11x / 1.02x -- not met: below ~96-128 wide the recipe's O(batch n) quantisation outweighs the O(batch n^2) GEMM saving. `test/bench_neural_net_fp4.py` reports the gates; CI job `fp4_kernels_x86` runs the snippet generic / AVX2 / AVX-512 (native or Intel SDE).

- **FP4 for the network: `fp4` / `mxfp4` / `nvfp4` weights, integer-SIMD kernels, and NVFP4 training** ([neural-net.md](neural-net.md), section "FP4").
  - `QuantizedNeuralNet(net, format="int8"|"fp4"|"mxfp4"|"nvfp4", quantize_activations=False)`: packed E2M1 codes (ONNX/PyTorch nibble order, rows padded to 32) with per-row float32 (`fp4`), OCP MX E8M0 blocks of 32 (`mxfp4`, floor rule) or NVIDIA E4M3 blocks of 16 plus a float32 tensor scale (`nvfp4`). `get_format`, `get_bits_per_weight` (mxfp4 4.25, nvfp4 4.5), `get_weight_bytes` now includes scales (int8: + 8 bytes a layer), `to_msgpack`/`from_msgpack` (`bff.quantized_neural_net`, bin fields, bit-exact; written by a small msgpack codec in `NetworkDocument.h` because the vendored nlohmann 3.6.1 has no `bin`), `get_kernel_name()`.
  - `internal/MlpFp4Kernels.h`: llama.cpp/ggml's MXFP4/NVFP4 dot kernels (MIT) re-expressed -- table lookup of `2 x E2M1`, int8 activations (W4A8) or FP4 activations (W4A4), `vdotq_s32` / plain NEON / AVX2 chosen at compile time, generic scalar otherwise; every variant bit-identical to the generic (four builds run by the test, AVX2 under Rosetta 2). This Mac's module compiles `neon-dotprod`. 256^3 GEMM: 520 us FP4 x int8 vs 1090 us double MatGemm; whole 24-256-256-128-8 pass at batch 1 1.4-2.4x faster than double, at batch 256 1.1x (tanh-bound). Standalone: opt-in `IMPBFF_WITH_AVX2`, `IMPBFF_WITH_ARM_DOTPROD`.
  - `NeuralNetTrainOptions.precision = "nvfp4"|"mxfp4"` (`internal/MlpFp4Train.h`): arXiv 2509.25149's recipe -- fprop/dgrad/wgrad on the FP4 kernel, 16 x 16 2-D weight scales, Hadamard on wgrad inputs, stochastic rounding of gradients, float64 master weights, first/last layer float64 -- and `NeuralNetTraining.get_quantized_network()` reproduces training's forward pass bit-exactly. Held-out: regression MSE 3.1x (nvfp4) / 3.5x (mxfp4) float64's; HMM surrogate MAE 1.09x / 1.08x.
  - Cross-checked against PyTorch 2.13 (torchao's E2M1 encoder, `pack_uint4`, `float8_e4m3fn`, RCEIL `to_mxfp`): 0 code differences. Output error vs float64 (W4A8, trained 3x64 net): fp4 4 %, mxfp4 13 %, nvfp4 8 % of the absmax; int8 0.9 %. Tests: `test_neural_net_fp4.py`, `test_neural_net_fp4_training.py`, `cpp_snippets/test_fp4_kernels.cpp`.

- **tttrlib's `NeuralNet` API is bff's now** (tttrlib removed it in `10858cf19`): concept page [neural-net.md](neural-net.md). msgpack stays the storage format; ONNX and safetensors are import formats only, and there is no ONNX writer.
  - `NeuralNet`: `from_onnx(bytes)`, `from_onnx_file`, `from_safetensors(bytes, hidden_activation)`, `from_safetensors_file`, `to_msgpack()`; one-sample `predict(x)`; `predict_derivatives(X, V, order)` (`y`, `J v`, `v^T H v`), `jacobian`, `hessian`, `backward`, `backward_derivatives`, `get_parameters`/`set_parameters` (copy on write), `get_n_parameters`, `get_layer_activation/weights/bias`. Arrays come back as managed views, 2-D where they have a shape. Derivatives run on the CPU; only `predict` is offered to the compute backend. The bytes typemap is now one macro for three typedefs: `MsgpackBytes`, `OnnxBytes`, `SafetensorsBytes`.
  - `internal/MlpGemm.h`: the `MatGemm` policy (Mat.h's blocked kernels) is used by `NeuralNet` and `train_neural_net`, and MlpCore's default stays portable. It matches the portable path to within 1e-12 relative, forward and backward. Measured without OpenMP: 2.35x the portable path at batch 256, 2.4x at 512.
  - `QuantizedNeuralNet` (`internal/MlpQuant.h`) is dynamic-range int8 inference, **rebuilt from the lost tttrlib spec, not recovered code**. Weights are 8x smaller; the error stays under 1 % of the output absmax on the fixtures and on a trained net. It runs 2.1x the portable double path, but only 0.9-1.3x the MatGemm path. The surviving descriptions of tttrlib's lost `MlpGemm.h`/`MlpQuant.h` work are kept in neural-net.md.
  - Fixtures are restored to `test/input/nn`; `test_bff_loads_an_onnx_model_without_tttrlib` now runs against them. New tests: `test_neural_net_derivatives.py` (39 passed; 1 skipped without `onnx`), `cpp_snippets/test_mlp_core.cpp` (tttrlib's C++ checks plus the MatGemm and int8 ones), `medium_test_neural_net_sklearn.py`, `expensive_test_neural_net_examples.py`. Also `bench_neural_net.py`, and the gallery section `examples/neural_net/` (differentiable building block, heat and Burgers PINNs). `test_docs_api_names` ignores `bff.neural_net`/`bff.hmm_surrogate`, which are format strings.
...
- **Free-energy landscapes and D from photons, photon by photon (Dingeldein & Covino, arXiv:2608.21061)**: `FRETLandscapeModel` (`FRETLandscape.h`, `FRETLandscapeGrid.h`), concept page [fret-landscape.md](fret-landscape.md).
  - SqRA generator and its symmetrisation (`sqra_*`), the QL tridiagonal eigensolver made public (`symmetric_tridiagonal_eigen`, moved unchanged from `DiffusionSolverKrylov.cpp` into `internal/TridiagonalEigen.h`), `NaturalCubicSpline`.
  - Batched normalised forward filter with an exact adjoint gradient (Daleckii-Krein divided differences, one `M x M` accumulator); priors; MAP `fit` with tttrlib's `i_lbfgs.h` (owner ruling: no copy; `fit` throws without `IMP_BFF_HAS_TTTRLIB`), preconditioned by the damped Laplace precision; `simulate`; `initial_guess`; `laplace` (empirical Fisher + prior); `posterior_marginals`/`sample_trajectory` (FFBS).
  - Tests: `test/landscape/` (18, ~2 s): brute-force expm, two-state MMPP by hand, gradients against central differences, Boltzmann stationarity. Two-well example (paper Fig. 2), 300 traces/1.78M photons: D = 1.71 +- 0.24 (true 1.5), barrier 5.50 +- 0.73 kT (true 5.01), 98% of the populated landscape within 1 sigma, photophysics within 1.5%.
...
- **HmmSurrogate and network training moved in from tttrlib (T-20260923-nn)**: tttrlib is now free of ML code; learned models live in bff even when their inputs are photons (AGENTS.md placement rule).
  ...
  - Build: `IMP.bff-lib` compiles as C++17 when tttrlib is linked (tttrlib's `TTTR.h` needs `std::any`). `internal/info.h` includes tttrlib's own `info.h` when bff links tttrlib, because both share the `TTTRLIB_INFO_H` guard and the reduced copy hid tttrlib's record-type macros in the unity build.
  - Tests: `test_neural_net_training.py` (7) and `test_hmm_surrogate.py` (17, including NumPy feature parity at 1e-12 and `HMM.evaluate`/`optimize` accepting the prediction), plus `test_neural_net`, `test_tttrlib_is_optional` and `test_vendored_headers`: 43 passed, 2 skipped (no GPU backend). Example: `examples/spectroscopy/plot_hmm_surrogate.py`. chisurf's `burst_h2mm/core/surrogate_bff.py` is the consumer.
...
- **PRD-136 Stages 5 & 6 closed (dye-timewarp / TITO): fragment gate, lifetimes, ensemble export, and consumer verification**:
  ...
  - **Stage 5 Fragment Gate (`s10_gate_fragment.py`)**: Gated 20 Å restrained fragment MD against 50 ns unrestrained whole-protein MD at `172L_A86` and `172L_A132` (Kabsch aligned to backbone). Surface contact fractions match within fragment restart noise (0.99 vs 0.83±0.17 at site 86, 1.00 vs 0.85±0.29 at site 132; PASS). Spatial density overlap (0.000–0.001) sits within fragment restart-to-restart noise floor (0.044–0.054; PASS). Site 132 passes internal rotational relaxation (|dC| = 0.398 <= 0.484 spread; PASS).
  - **Stage 5b Lifetime Simulation (`s12_lifetime.py`)**: Evaluated Dexter PET quenching lifetimes across all 236 finished trajectories (39 sites). On T4L (15 sites: 10 pAcF + 5 maleimide), simulated `tau_slow` correlates with measured `tau_x` from `pinn_table.csv` (Pearson r = +0.50, Spearman rho = +0.47; maleimide alone r = +0.31).
  - **Stage 6 Ensemble Export (`s15_export_ensemble.py`, alias `s11_export_ensemble.py`)**: Built rotamer ensemble exporter writing conformer coordinates, weights, empirical jump counts, and reversible Markov generators `L`. Exported `.drot.pto` libraries and verified lossless round-trip via `IMP.bff.read_probe_rotamer_drot` and `IMP.bff.ProbeRotamerEnsemble`.
  - **Stage 6 Consumer Verification (`s16_consumers.py`, alias `s12_consumers.py`)**:
    - **PRD-119 Check**: Markov generator `L` reproduces direct MD dipole autocorrelation `C(t)` on free dye `A48_C1R` with max |dC| = 0.058 <= 0.10 (PASS; tau_MSM = 1.24 ns vs tau_dir = 1.30 ns) and on representative surface-bound site ensembles (max |dC| <= 0.074; PASS).
    - **PRD-99 Check**: Mapped Accessible Volume (AV) failures across 39 sites: 10 sites (25.6 %) show severe surface sticking (>85 % contact, e.g. T4L 19/22/36, MalE 72/92/95/233) and 7 sites (17.9 %) partial sticking (60–85 %), where steric AV severely overestimates mobility and misrepresents localized quenching.
    - **PRD-135 Check**: Simulated lifetimes on T4L achieve Spearman rho = +0.47, exceeding the PINN baseline (+0.35). Residual anisotropy across 39 sites matches experimental mean (sim 0.161 vs meas 0.163, bias -0.002, RMSE 0.074), with maleimide Spearman rho = +0.11 (matching PINN baseline +0.09).
  - **Rotamer Library Closing Queue (PRD-116)**: Monitored `run_queue_closing2.sh` on `heinzehub` GPU 1. `A48_C1Q_r5` is progressing stably at 726 ns/day (step 13.8M / 100M). GPU 0 remains free.
...
## 2026-09-14 — a built description is a live model; ChiSurf is a view on it

Owner ruling: the fit graph lives in BFF and ChiSurf only gets a view; models stay abstract, as data. `ModelSearchSpec::get_model()` builds once and, after new data, scalars or overrides, rebuilds the graphs **over the same parameter ports** (values kept and clipped into re-derived bounds, locks/releases and the selected topology carried). `MultiStructureModelSearchProblem` gained `select_structure` (values untouched), `get_structure_parameter_ids` (what a topology's graph reads, recorded from `#id` references), user `set_parameter_locked`/`set_parameter_released` honoured by fits, starts, searches and the BIC/AIC parameter count, and `fit_active_structure`. Descriptions may write a bound as `null` (unbounded); lifetime amplitude ratios to the fixed reference are now unbounded above, because a ceiling of 1 made the answer depend on which component sat in the reference row (owner: "the ordering is just for the user"). Presentation (labels, groups, the primary measurement, scalar defaults as rules over `decay_dx` etc.) is a `presentation` block read by applications via `get_description_json()`/`evaluate()`. Goldens unchanged; gate 708 passed. ChiSurf side: one generic `chisurf.core.models.description.DescriptionModel`, listed as `chisurf.core.models.description.tcspc_lifetime`. Not yet ported: response preprocessing (lamp background, IRF window), FCS/anisotropy/global views.

## 2026-09-14 — equation catalogues as families, over any number of coordinates

Owner: parsed models stay abstract in BFF, catalogues stay ChiSurf YAML, entries compete on BIC. `ModelSearchSpec::set_equations(json)` expands a catalogue against the generic frame `data/model_search/equations.json` (no equation, no experiment): each entry is a structure; a variable naming a coordinate of the bound `curve` measurement is an axis (`@curve.<name>`, now valid in any description), every other variable a parameter shared by name (unbounded unless bounded, freed unless held); `moves: "all"` links every entry to every other. A correlation curve (`x`) and an image-correlation carpet (`xi`, `psi`, `tau`) are the same code. Expansion copies the frame before growing the document (an ordered-map reference into it dangled and segfaulted). ChiSurf: ParseFCSModel, ParsePCFModel, ParseStoppedFlowModel and ImageCorrelationModel are catalogue views; all 50 FCS entries and the ICS 3D/membrane/Gaussian carpets reproduce the deleted classes' curves to 1e-9.

## 2026-09-14 — a generic Convolution node; TCSPC equation models ported

`Convolution` convolves whatever curve arrives with a bound response (causal, `np.convolve(..., "full")[:n]`), shifted by its `timeshift` input and normalised to unit sum through `internal/ResponseFunction.h`, the one preparation `TCSPCDecay` now also uses. The kernel is tttrlib's new header-only `convolve_causal_ad` (vendored byte-identical). `TCSPCDecay` gained `curve_from_port`: it takes an already convolved curve and runs only its instrument stage (scatter, pile-up, scale, background, linearisation, clamp), so that stage has one implementation. Frame `equations_convolved.json` puts a catalogue equation in front of both; expansion now keeps a frame's own parameters and free list. An autoscaled `n0` is published to the canonical owner (it was written to the follower, so a caller never saw it). **`shift_lamp_ad` changed:** a channel half outside the response now keeps the inside sample's share instead of being blanked -- the blanking made the shifted response discontinuous in the shift (chisurf's `shift_array` was fixed the same way) and was the only difference from ChiSurf's ParseDecayModel under a timeshift; tttrlib's `test_fsconv` pin updated, installed tttrlib not rebuilt. ChiSurf ParseDecayModel reproduced to 1e-9 over 9 equations x 4 instrument settings.
- 2026-09-15 — Convolution modes: `causal` (default, unchanged), `periodic` (adds the spill of every earlier excitation, period in samples, fractional periods interpolated -- tttrlib `convolve_full_ad` + `fold_periodic_ad`), `centered` (`np.convolve(..., "same")` for symmetric kernels). A description `config_rules` entry may be `{"options": [...], "index": rule}` to pick a named setting from a numeric switch; `equations_convolved` uses it for an optional `periodic` flag (period from the `period` scalar and the decay's channel width). ParseDecayModel parity unchanged (causal default).
- 2026-09-15 — IRF preparation in the graph: `internal::ResponsePreparation` (subtract a constant background, clip at zero, zero outside `[start, stop)`, then shift and normalise -- ChiSurf's `_process_irf` order) shared by `TCSPCDecay` and `Convolution`; active when a node gets `response_range` or a wired `response_background` input. `tcspc_lifetime` and `equations_convolved` declare `instrument.response_background` (held, [0, response_max]) and `response_start`/`response_stop` scalars in ns (`floor(t/dt)` channels, as ChiSurf). tcspc_lifetime golden regenerated for the added id (values move <1e-6, rewards 1e-11). ChiSurf's classic LifetimeModel and the lifetime view agree to 1e-13 with lamp background, window, shift, scatter and background set -- after fixing ChiSurf's `Convolve.__init__`, which lost dt and the convolution/IRF stop (1 ns) whenever a decay had no reader.
- 2026-09-15 — TCSPC background pattern: `TCSPCDecay` takes a measured background decay (`background_pattern` role) with `background_times` [t_background, t_decay] and `shift_background_with_response`; ChiSurf's order and arithmetic (after scatter, before pile-up and scale: shift with the IRF, `n_bg = sum/t_bg*t_exp`, model rescaled to `max(sum(data) - n_bg, 1)`, pattern to `n_bg`). Optional dataset and scalars in `tcspc_lifetime` and `equations_convolved`. ChiSurf classic LifetimeModel with a background curve and the view agree to 1e-9. The basis is not the Jacobian while a pattern is set.
- 2026-09-15 — TCSPCDecay `convolution_mode` (`periodic` = fconv_per_cs, `single` = fconv, tttrlib's scalar body now the header template `fconv_ad`) and `convolve` (off: the ideal decay, periodic tail in periodic mode, as ChiSurf's decay_without_irf). tcspc_lifetime scalars `periodic_excitation`, `convolve`. Parity with ChiSurf's classic exp / no-convolution modes to 1e-9.
- 2026-09-15 — Modelled IRF: node `GeneralizedNormalCurve` (area-scaled generalized-normal peak on an even axis, floor); `TCSPCDecay`/`Convolution` `response_from_port`; dataset statistic `<slot>_rise_index`. tcspc_lifetime and equations_convolved: IRF dataset now optional, `generated_response` flag, parameters instrument.irf_width / irf_shape / irf_position (fixed; position seeded at the decay's 10% rise), presentation `required_unless`. Golden gains the three held ids only. Parity with ChiSurf's generated IRF to 1e-9.
- 2026-09-15 — `tcspc_polarized` description (data only): the lifetime family's instrument with a rotation axis m 1..2, rotation.amplitude/time, anisotropy.r0/g/l1/l2, and a `polarization` scalar choosing VM/VV/VH/VV-VH for PhotophysicsAnisotropySpectrumNode feeding TCSPCDecay's spectrum port. ChiSurf classic polarised LifetimeModel reproduced to 1e-9 for VV, VH, VV/VH.
- 2026-09-15 — DNL: `linearization_table` (ChiSurf's compute_linearization_table in C++, five windows; ChiSurf's function now forwards to it) and TCSPCDecay role `linearization_curve` + `linearization_smoothing`/`linearization_window` (x range = fit window, reverse). tcspc_lifetime, tcspc_polarized, equations_convolved declare the measured DNL curve and its smoothing. Classic parity to 1e-9, forward and reversed.
- 2026-09-15 — Presented outputs: TCSPCDecay publishes its spectrum on an optional `spectrum` output; `MultiStructureModelSearchProblem::get_structure_port`. Descriptions present `distributions` (lifetime spectrum), `statistics` (species/fluorescence averaged lifetime), `reference_modes` and an `axis`; ChiSurf renders them generically (distribution plot, output parameters, shared photon reference modes).
- 2026-09-14 — PRD-142 step 1: `include/BayesianMeasuredResponse.h` -- a measured response prepared for a Bayesian decay fit (soft background over its support, clamped Fourier shift, unit sum), the exact bin-integrated periodic kernel as a caller-owned `BayesianPeriodicKernel` (column FFTs kept), the basis and its tangents in background and shift. Ported from ucfret `s89_cpp`; ucfret's gates against the torch prototype pass unchanged through this header (bases 5e-15, count Jacobian 4e-15, fit 2.2 s). Test `test/decay/test_bayesian_measured_response.py` (header-only driver via `test/decay/bayesian_cxx.py`, pocketfft from tttrlib's thirdparty): tangents vs differences 2e-9, the clamp's mask must matter (5.8e-4), column sums 1e-15, the exact kernel vs an independent integration 2.5e-7 while a centre-sampled kernel misses by 0.66, a whole-channel shift is a roll to 3.5e-16.
- 2026-09-14 — PRD-142 step 2: `include/BayesianGoodnessOfFit.h` -- Poisson deviance per dof per histogram and overall, the runs test on residual signs (statsmodels `runstest_1samp`, cutoff mean, no correction, ported line for line), the deviance reference measured by drawing Poisson data at the fitted means. ucfret's CBM56 Rule 0 now runs through it with identical numbers. Test `test/decay/test_bayesian_goodness_of_fit.py`: runs p = statsmodels to 3e-17 on five sequences; data from the model pass (z +0.6); means 5 % off fail (z +104); a slow 3 % wave fails the runs test (p 1e-9). `test/decay/bayesian_cxx.py` gains `stdin=`.
- 2026-09-15 — Links across models: a canonical parameter may follow a port of another model (a ChiSurf link across fits); it is held like a lock (never freed, never seeded, not counted), survives a rebuild, and following another parameter of the *same* model is refused at validation.
- 2026-09-14 — PRD-142 step 3: `include/BayesianDecayModel.h` -- the experiment as typed tables (`BayesianDecayExperiment`: scopes, parts, responses, variables, fixed values, P-spline settings, named arrays, the periodic kernel on its grid) read once by `bayesian_decay_experiment_load` from the manifest format ucfret's emitter writes; theta unpacked through identity/log/logit/ALR/sum-to-zero; the amplitudes of every physics channel (donor, sensitised and direct acceptor blocks, polarised mixing, instrument), their analytic Jacobian, expected counts and the count Jacobian (live columns per histogram), on a persistent thread pool (`internal::BayesianThreadPool`, `BFF_BAYESIAN_THREADS`). Converted mechanically from ucfret `s89_cpp` (manifest lookups to table fields); ucfret's gates against the torch prototype pass through it unchanged (amplitudes 9e-16, Jacobians 6e-16 / 4e-15, fit 2.2 s), its negative controls fail as before. Test `test/decay/test_bayesian_decay_model.py` on a synthetic experiment (`bayesian_decay_synthetic.py`: D0, DA, reference dye, two polarised detectors): both Jacobians vs central differences at the best of four steps, 8e-8 / 7e-8 on all 51 columns; a column planted 1e-4 off is caught at 1e-4; the FRET sample handed the donor-only scope changes by 135 % while the donor-only sample does not move.
- 2026-09-15 — Axis bounds may be expressions over optional scalars: `from`/`to` of a template axis are evaluated against the optional-scalar defaults overlaid by the caller's values, and the template is re-expanded when those change (rebuild over the same ports). tcspc_lifetime/tcspc_polarized take `max_components` (default 3), so a fixed fit may ask for more components than a search offers by default.
- 2026-09-14 — PRD-142 headers no longer break `IMP/bff.h` (reported by the model-search session): `BayesianMeasuredResponse.h` and `BayesianDecayModel.h` compile only where pocketfft is on the include path (`__has_include("pocketfft/pocketfft_hdronly.h")`, reached through tttrlib's `thirdparty/`), and declare nothing otherwise -- the IMP module build today; the torch-thresholded soft floor is renamed `bayesian_soft_positive_thresholded`, since `BayesianFisherScoring.h` already defines `bayesian_soft_positive` (the unthresholded floor; the two differ by under `soft * 2e-9`); `M_PI` replaced by a literal (MSVC). Checked: every `Bayesian*.h` plus `Optimization.h` in one translation unit compiles without and with pocketfft, no warnings; the three decay tests and ucfret's CBM56 gates pass unchanged. To make the headers live in the module, the build needs tttrlib's `thirdparty/` on its include path -- not done here.
- 2026-09-14 — PRD-142 step 4: `include/BayesianDecayPosterior.h` -- the prior (per-variable families from the experiment's table, the spectrum's continuity prior, the order-2 P-spline on log p(R/R0)) with analytic gradient and Hessian, the Poisson likelihood, `bayesian_decay_evaluate` (gradient and Fisher matrix on each histogram's live columns), `bayesian_decay_fit_node` (Fisher scoring through `DampedNewton::line_search_below`), the Laplace evidence, the covariance and p(R/R0) with its delta-method sd; guarded like the model header. ucfret's CBM56 fit now runs entirely through bff's headers and still lands on the polished Python mode (B7/B8, 2.2 s). Test `test/decay/test_bayesian_decay_posterior.py`: prior gradient / Hessian vs differences 1.6e-12 / 1.4e-12, posterior gradient 1.7e-7; data drawn from the synthetic model at a known point and fitted from a perturbed start converge (29 steps) and pass the goodness-of-fit test (z +1.5); the same counts with a 20 % ripple are rejected (z +33.7). A lifetime stretch of 20 % was tried first as the rejected case and is not one: the free lifetime spectrum absorbs it (z +2.1).
- 2026-09-14 — PRD-142 test 5: `test/decay/expensive_test_bayesian_decay_cbm56.py` -- the CBM56 measurement (ucfret fixture, named by `BFF_CBM56_FIXTURE`, skipped without it) fitted by a driver built from bff's headers alone, against the torch prototype's Newton-polished mode: converged in 186 steps, evidence +1e-6 nats, p(R/R0) 6.8e-6 and its sd 3.3e-6 of the peak, D/dof 6.4e-7, overall z +2.37 (Rh110 VH failing, as recorded in ucfret), 2.26 s wall. PRD-142 steps 1-5 done; step 6 (a subcommand) needs a module build that has tttrlib's `thirdparty/` on its include path.
- 2026-09-15 — Lifetime mixtures: a model publishes named outputs (`MultiStructureModelSearchProblem::publish_output`/`get_output_port`, a relay that follows the active topology and survives rebuilds; descriptions declare `outputs`), a description reads caller-bound external ports as `$<name>` (`ModelSearchSpec::set_port`, declared under `ports`), and the node `LifetimeSpectrumMixture` joins K lifetime spectra weighted by normalised fractions. `tcspc_mixture` (axis bound by `number_of_models`, fraction 0 held) reproduces the lifetime model with the combined spectrum to 1e-10 and follows changes in the mixed models live. tcspc_lifetime/tcspc_polarized publish `lifetime_spectrum`.
- 2026-09-15 — FRET moves to BFF: five description families (tcspc_fret_gaussian, _discrete with the new DiscreteDistances node, _worm_like_chain, _saw_nu, _ising_chain), each donor -> distances -> FRETSpectrumNode -> TCSPCDecay with the lifetime instrument; distance nodes take JSON settings (`axis` values, or `axis_range` [min,max,n] + `axis_scale`); FRETSpectrumNode publishes `fret_rates` (p, k_FRET); problems read published outputs evaluated (`get_output`), and `@name` publishes a node's own output. ChiSurf parity with the classic FRET models is 1e-9 for every family (chisurf test/fitting/test_fret_views.py). Follow-ups agreed with bayesian.decay: analytic d(weights)/d(family parameters), kappa2 regimes.
- 2026-09-15 — Distance weights Jacobians for BayesianDecayModel (agreed with bayesian.decay): `get_weights_jacobian()` row-major n_axis x n_params with `get_parameter_names()` on GaussianDistances (analytic, both kernels, both normalisations, sign(x) for |mean|/|amplitude|, a series for small shapes), DiscreteDistances (analytic) and PolymerDistances (central differences, relative step 1e-5; half-step difference <= 1e-9 relative; residue count and unused linker width columns are 0). PolymerDistances gains `normalize_weights`.
- 2026-09-15 — Static kappa^2 in FRETSpectrumNode: `orientation` dynamic (scalar) / static (distribution on `kappa2_distribution`) / static_isotropic (128-point isotropic density over [0.01, 4]); `kappa2_bins` 0 makes every (distance, kappa^2) pair a species (ChiSurf slow-mode exact path, 1e-9), N histograms apparent distances into N linear bins (default 512 in the FRET families; within 2e-3 of exact). FRET descriptions take `static_orientation` and `kappa2_bins`.
- 2026-09-14 — PRD-143 step 1: the Bayesian decay headers use bff's own building blocks instead of copies -- `BayesianPSplinePrior` (the P-spline prior, gradient, Hessian), `BayesianTransforms.h` (unpacking; the sum-to-zero basis is the experiment's `Q`), `bayesian_poisson_score_blocks` (new, in `BayesianFisherScoring.h`: the Poisson score accumulated per block of rows on its own columns, with a caller-supplied parallel-for), `bayesian_laplace_log_evidence`, `bayesian_delta_variance`; one soft floor, `bayesian_soft_positive(x, s, threshold)` with `BAYESIAN_TORCH_SOFTPLUS_THRESHOLD` (the `_thresholded` copy and `bayesian_softplus*` removed). Every fixed-point gate of ucfret's CBM56 parity passes unchanged (<= 4e-15; log posterior 4e-7 nats), the four decay tests and the s89 simulated regression pass, all `Bayesian*.h` compile together with and without pocketfft. **What does not pass**: `expensive_test_bayesian_decay_cbm56.py` -- the fit from the start now reaches the SECOND CBM56 mode (evidence -0.715 nats, p(R/R0) 6.7 % of its peak apart). Not caused by the change: with the previous headers, the same start moved by 1e-12 reaches the second mode by the identical path (204 steps, -0.7134). Which of the two modes one fit reaches is decided by rounding; the earlier pass was that luck. Keeping the best mode needs several starts (tpeulen's decision pending).
- 2026-09-14 — PRD-143 inventory #10, #11: `BayesianGoodnessOfFit.h` removed -- its measured Poisson reference and per-histogram summary are now `poisson_goodness_of_fit` in `FitStatistics.h`, computed with the header's own `poisson_deviance` and `runs_test` (cutoff at the mean of the residual signs = statsmodels `runstest_1samp(cutoff='mean')`); test renamed `test/decay/test_poisson_goodness_of_fit.py` (runs p = statsmodels to 3e-17, true model passes, 5 % off and a ripple fail). The decay model's loops run on `internal/ThreadPool.h` (one pool per process, `BFF_BAYESIAN_THREADS`), `internal::BayesianThreadPool` removed; the per-thread serial switch concurrent fits need stays as a function. ucfret's CBM56 gates pass, Rule 0 identical; the fit 1.8 s (the pool's spinning workers). Degenerate case changed: an all-one-sign residual sequence now gives runs p 1 (`runs_test`) instead of NaN.
- 2026-09-15 — Distance kernels exist once, for any number type (include/internal/DistanceKernels.h: normal, generalized normal, Gaussian-cloud distance, WLC, WLC+linker, SAW-nu, Ising); Distributions.cpp and PolymerChain.cpp call them with double (outputs bitwise unchanged), and GaussianDistances/PolymerDistances::get_weights_jacobian evaluate them on tttrlib's vendored forward-mode Dual<GradVec<N>> (internal/Dual.h, GradVec.h). Exact derivatives of what is output, replacing the hand-derived Gaussian and the polymer finite differences; checked against best-of-steps Richardson differences (WLC 5e-10, WLC+linker 5e-10, SAW-nu 8e-11, Gaussian <1e-7, Ising 2.5e-7 = the reference's roundoff floor) with a planted 1e-5 error caught.
- 2026-09-14 — PRD-143 inventory #12: the decay model's polarised mixing goes through `PhotophysicsPolarisation.h` -- `anisotropy_weight` for the VV/VH weight (2 - 3 l1, -1 + 3 l2) and `mix_polarised` for donor and acceptor, in the amplitudes and in the Jacobian's forward values; the derivatives of that linear form stay where they are used. ucfret's CBM56 gates unchanged (amplitudes 9e-16, Jacobians 6e-16 / 4e-15), bff's decay tests pass.
- 2026-09-15 — De-duplication (with bayesian.decay): the distance kernels use tttrlib's vendored Dual.h + GradVec.h (GradVec.h newly vendored, byte-identical, pinned in test_vendored_headers); the dual special functions (I0, gamma) stay with the kernels in bff; ForwardDual.h is gone. The binned static-kappa^2 path of FRETSpectrumNode uses the existing outer_product_histogram instead of its own histogram.
- 2026-09-15 — PRD-143 step 3 (IRF preparation): `internal/ResponseFunction.h` is the one preparation for the graph and the Bayesian decay model. New there: `ResponseBackground::fraction_of_support` beside ChiSurf's `constant` (the constant path is unchanged, byte for byte), `clean_response_background_derivative`, and `normalize_unit_sum` (unit sum with tangents through the quotient rule). `bayesian_response_basis` now cleans, normalises the shifted response and normalises every convolved column through these; its own copies are gone. The shift keeps one implementation per mode: linear interpolation (`shift_lamp_ad`, ChiSurf) and the clamped Fourier shift (FFT header, where pocketfft is). ucfret's CBM56 gates unchanged (bases 5e-15, count Jacobian 4e-15, both negative controls still fail), bff decay tests pass. ChiSurf parity for the constant path is by construction (untouched) -- to be confirmed on the next module build.
- 2026-09-15 — PRD-143 #22 and #21 put to tpeulen instead of done: the TCSPCDecay "instrument stage" overlap is two one-line loops in different spaces with different parameterisations, so an extracted function would not remove a copy (the real merge is a design change of the decay model); the convolution merge needs tttrlib changed first.
- 2026-09-15 — PhotophysicsTransferKinetics.h: THE excited-state kinetics between two chromophores transferring both ways (agreed with bayesian.decay for PDDEM and the Bayesian transfer tensors). Putzer's form, exact at the degeneracy; components with a kind flag (exponential / t e^{-kt} below eps |l1|, eps a parameter) and a named ChiSurf-regularised mode for pddem parity; per-unit p0/m entry points, decay form, exact components Jacobian on tttrlib duals; transfer_kinetics_spectrum reads the light path from PhotophysicsCrosstalkMatrix (which gains crosstalk_apply_mixing_jacobian and crosstalk_row_shares). PhotophysicsTransferKineticsNode wraps it. R0/tau0 are documented as one pair on FRETSpectrumNode and here. Checked against scipy expm (1e-10, degeneracy included), the one-way donor/acceptor closed form, and ChiSurf pddem_rates (1e-12).
- 2026-09-15 — tcspc_pddem: chromophores A and B (lifetime spectra), Gaussian distances -> FRETSpectrumNode fret_rates -> PhotophysicsTransferKineticsNode (f_ab, f_ba, pure fractions, excitation/emission as crosstalk entries) -> polarisation -> TCSPCDecay. Reproduces ChiSurf PDDEMModel to 1e-9 in the ChiSurf-regularised mode and 1e-6 in exact mode (chisurf test_fret_views.py).
- 2026-09-15 — Acceptor density FRET (renamed from ChiSurf's "distributed acceptor"): FRETAcceptorDensity.h (C0, reduced density, quenching factor, efficiency, the folded quenched decay), node AcceptorDensityDecay (publishes efficiency and absolute density), description tcspc_fret_acceptor_density with the dimensionality 1/2/3 as a searched axis, instrument through Convolution -> TCSPCDecay(curve_from_port). Reproduces the classic model to 1e-9 (chisurf reference frozen before its deletion); chisurf dimensionality.py became acceptor_density.py forwarding here.
- 2026-09-15 — PRD-143 step 4 (#21, tpeulen: "ok change tttrlib for 21, but keep names neutral"): the exact bin-integrated periodic kernel lives in tttrlib as `periodic_decay_kernel(out, n_period, dt, tau, order)` (`modules/spectroscopy/decay/include/PeriodicDecayKernel.h`, orders 0 = exp(-t/tau) and 1 = t exp(-t/tau), the degenerate transfer-kinetics component; tttrlib test `test/cpp/test_periodic_decay_kernel.cpp`), vendored verbatim as `include/internal/PeriodicDecayKernel.h` and pinned in `test/test_vendored_headers.py`. `BayesianPeriodicKernel` takes its columns from it; its own formula is gone. ucfret's CBM56 gates pass (bases 2.7e-14, from K(0) = 1 - sum of the rest). What stays in bff is the FFT application of the kernel to a response, which only this model does.
- 2026-09-15 — PRD-143 #22 (tpeulen: "bayesian way better, but pileup missing, chisurf changes"): `include/internal/TCSPCInstrument.h`, the one instrument stage -- scale, and scatter / background pattern / uncorrelated background as fractions of the fluorescence total with given unit-sum shapes (`tcspc_instrument_components`), then Coates pile-up through the vendored `add_pile_up_to_model_ad`, the background after pile-up, the DNL multiply (`tcspc_instrument_detection`; `tcspc_instrument` = both); closed-form derivatives (`tcspc_instrument_components_jacobian`); ChiSurf's absolute scatter and background as fractions (`tcspc_instrument_fractions_from_absolute`). Templated on the number type. The Bayesian decay model applies it in its basis's amplitude space (response and flat background as unit vectors) -- ucfret's CBM56 gates unchanged. Test `test/decay/test_tcspc_instrument.py`: the two spaces agree to 3e-16, derivatives 2e-10 / 1e-9 with a planted 1e-4 caught, ChiSurf's absolute form reproduced to 1e-18, ChiSurf's order with pile-up and DNL against a hand write-out 2e-16, and the background before pile-up must differ (1e-5). Next: TCSPCDecay and the ChiSurf descriptions onto it (model-search session); pile-up and DNL in the Bayesian model (channel-space detection per histogram).
- 2026-09-15 — TabulatedDistances (given distributions on a common axis mixed by fractions, relative threshold, analytic weights Jacobian) and tcspc_fret_tabulated (distributions and axis bound as $ports): ChiSurf's structure-ensemble FRET model is this family plus Python that loads PDBs and computes AV distributions (non-hot, by the owner's callback rule), reproducing the classic model to 1e-9. AcceptorDensityDecay no longer computes efficiency/density per evaluation: descriptions declare Python "reports" the application evaluates on demand.
- 2026-09-15 — Pile-up and DNL in the Bayesian decay model (tpeulen: "pileup missing"): `bayesian_decay_expected_counts` applies the shared stage's `tcspc_instrument_detection` per histogram when the experiment carries a `pile_up` block (repetition rate, dead time, measurement time; the factors from each histogram's own counts) or a `linearization` array -- pile-up on fluorescence and scatter, the background after it, the DNL multiply; count Jacobian `g J_raw + (lin - g) J_bg`. Loader reads both. Test `test/decay/test_bayesian_decay_detection.py` (synthetic experiment, now with `pile_up=`/`linearization=` options): DNL exact (6e-17), pile-up a factor on fluorescence with the background off (1e-16) and a 1.2 % effect, the background not piled (a remainder of 3e-5 of the peak, as it must be), the Jacobian 7e-8 on every column. ucfret's CBM56 gates (no pile-up block) unchanged.
- 2026-09-15 — Adam once (tpeulen: "ADAM? Random gradien descent?" / "could be generally useful"): tttrlib `modules/math/include/AdamUpdate.h` (`tttrlib::adam_update`, `AdamState`; Kingma & Ba Algorithm 1; tttrlib test bit-identical to the algorithm, converges on a six-decade quadratic, the uncorrected variant must differ), vendored as `include/internal/AdamUpdate.h` and pinned in `test/test_vendored_headers.py`. **Still two private copies to switch**: `src/ModelSearchSelfPlay.cpp` (inline loop, model-search session) and tttrlib `modules/math/src/NeuralNet.cpp` (`adam_step`; that file has uncommitted changes from another session, so not touched). Measured use on CBM56 (ucfret `s89_cpp/cbm56_adam.cpp`): 1500 full-gradient steps from the poor start, then Fisher scoring, end 0-6 nats below the best of three starts where scoring alone ends 46-78 below; best reached on 2 of 10 replicates (stochastic half-batches: 2 of 10), ~45 s per replicate.
- 2026-09-15 — PRD-143 #22 (owner: "bayesian way better, but pileup missing, chisurf changes"): TCSPCDecay runs its instrument through internal/TCSPCInstrument.h (written by bayesian.decay): scatter, background pattern and background are fractions of the fluorescence total, n0 counts per unit decay, ChiSurf's order (components, Coates pile-up, background after pile-up, scale, DNL). Autoscaling is the weighted least-squares factor of the whole unit curve (the background scales with it). Every TCSPC description migrates background initial/upper to fractions, labels its instrument parameters as fractions and publishes `fluorescence_total`; `tcspc_fractions_from_absolute` exposes the conversion. BFF tests pinning the absolute meaning were rewritten against the stage (a basis test that never set a period and compared inf with inf now sets one).
- 2026-09-15 — Self-play Adam onto the shared kernel (bayesian.decay's request after "Adam once"): `src/ModelSearchSelfPlay.cpp` drops its inline loop for `tttrlib::adam_update` (state reset once, one step per epoch). Training errors of the self-play fixture (400 episodes, [64,64], 200 epochs) bit-identical before and after; `test/mcts/test_self_play.py` 7 passed. Remaining private copy: tttrlib `NeuralNet.cpp` `adam_step`.
- 2026-09-15 — PRD-143 #22 closed on both sides (model-search session): `TCSPCDecay` runs on `internal/TCSPCInstrument.h` (e422de0), all TCSPC descriptions take scatter/background as fractions and publish `fluorescence_total`, `tcspc_fractions_from_absolute` exposed; ChiSurf converted (chisurf ffae4cac5, `core/fluorescence/tcspc/instrument.py` both directions); bff gate 691 passed. Self-play Adam on `tttrlib::adam_update` (892f3fe), bit-identical training errors. The one private Adam left: tttrlib `NeuralNet.cpp` (`adam_step`, a file with another session's uncommitted changes).
- 2026-09-15 — PRD-143 #27 (tpeulen: "Optimizer (the algos) should sit in tttrlib, if header only"): `CholeskyFactor`, `cholesky_solve`, `DampedNewton` moved to tttrlib `modules/math/include/DampedNewton.h` (with its test), vendored as `include/internal/DampedNewton.h` and pinned; `Optimization.h` is now aliases only; `bayesian_log_det_spd` forwards to `log_det_spd` -- the Cholesky factorisation exists once. Fixed on the way (a defect from 4b183f7, found by the new tttrlib test): `line_search_below`'s default did not backtrack on the first step. ucfret's CBM56 gates and the s89 simulated regression unchanged; decay tests pass.
- 2026-09-15 — PRD-143 #18a (tpeulen: "#18 should move"): `include/BayesianTransferTensors.h`, the basis the transfer maps are projected onto -- `bayesian_log_lifetime_grid` (per decade, both ends excluded), `bayesian_gaussian_response` (sampled, unit sum), `bayesian_transfer_basis` (defaults of ucfret's `s80.build`: 0.075 ns at 0.10 of the window, 0.05 ns to three windows, 10 per decade). No new basis code: it is `bayesian_response_basis` with a new option `BayesianResponseOptions::remove_background` (false: the soft floor is skipped; it moved a Gaussian's tails by 9e-5 of the peak). Against the Python prototype's cached CBM56 basis (ucfret `s89_cpp/transfer_gate.cpp`): grid 6e-15, response 7e-16, worst of 33 columns 1.2e-13 of its peak; negative control with the floor on fails (9.2e-5). Test `test/decay/test_bayesian_transfer_basis.py`: the CBM56 grid (31 lifetimes), column 0 the response untouched (floor-on control > 1e-8), unit sums, every column the time-domain circular convolution (no FFT) to 1e-12. Bayesian decay tests 28 passed.
- 2026-09-15 — ChiSurf keeps its scatter and background (tpeulen: "users are too much used to the old way the scatter was handled ... the new way must be transparent/hidden from the user"): `TCSPCDecay::set_instrument_units` / config `instrument_units`, "fractions" (default, the stage's own) or "counts". In counts the absolute scatter is converted to the stage's fraction at every evaluation (`tcspc_instrument_fractions_from_absolute`, the one conversion), the background is counts per channel added after the scale and taken off the data before autoscaling, and a data-derived background pattern keeps ChiSurf's rescaling of the model to n_fl -- so a held count keeps its meaning while the decay changes. Every TCSPC description is back in counts (labels, `<slot>_min` start, `<slot>_max` bound) with `instrument_units: counts`; `fluorescence_total` stays published. The Bayesian decay model keeps fractions. Tests: `CountsTests` in test/decay/test_tcspc_decay.py (classic formula over two decays at 1e-10, classic autoscale); description tests back to counts. Gate 694 passed.
- 2026-09-15 — PRD-143 #18b: the map projection. tttrlib `RidgeProjector` (`DampedNewton.h`: ridge least squares on a kept `CholeskyFactor`, lambda absolute or relative to the mean diagonal of `A'A`; tested against elimination, a collinear pair, scale invariance with an absolute lambda as the negative control), re-vendored as `internal/DampedNewton.h`. `BayesianTransferTensors.h` gains `bayesian_transfer_projector` (`BAYESIAN_TRANSFER_RIDGE = 1e-8`, ucfret `s79._ridge`). Against numpy on 31 CBM56 rotational-map targets (ucfret `s89_cpp/transfer_gate.cpp`): coefficients 1.3e-7, reconstructed decay 2.0e-12 of the target peak; lambda x10 fails. The plan's 1e-9 on coefficients was unreachable -- condition 2.7e9, numpy LU vs scipy Cholesky alone 5.8e-8 -- so the gate is 1e-6 / 1e-11 from that measured floor. Test `test_bayesian_transfer_basis.py`: known coefficients within the ridge bias bound. Remaining duplicate: the Eigen Tikhonov branch of `crosstalk_invert_mixing` (compiled source), to move onto `RidgeProjector` at the next module build.
- 2026-09-15 — PRD-143 #18c: `bayesian_transfer_rate_maps` (BayesianTransferTensors.h) -- maps of an added decay rate onto the transfer basis, one function for donor quenching (rate `(1/tau_ref)(R0/R)^6`, the (R0, tau_ref) pair) and rotational depolarisation (`1/rho`); columns at the shortened lifetimes through `bayesian_response_basis`, scaled by the kept light `tau_s/tau`, ridge-projected. Against the Python CBM56 maps (ucfret `s89_cpp/transfer_gate.cpp`): S_R (128 distances) and S_rho (11 rotational times) decays 4.8e-12 of their peaks, coefficients 5e-8; tau_ref 0.1 % off fails at 9.4e-4. 139 maps in well under a second (Python: minutes). Test additions: rate 0 is the identity within the ridge bias bound, kept light 1/(1 + k tau) to 5e-8 (unscaled control 0.996). 37 decay/vendored tests pass.
- 2026-09-15 — Crosstalk ridge onto the shared solve (bayesian.decay, PRD-143 #18b): `invert_block`'s `ridge > 0` branch in src/PhotophysicsCrosstalkMatrix.cpp uses `tttrlib::RidgeProjector` (vendored DampedNewton.h; absolute lambda, factored once, projected per item) instead of its own Eigen gram + LU. Cholesky for LU: 2e-14 relative difference on a 3x5 mixing with 40 items; test/test_crosstalk_matrix.py and the decay/mcts/minimizer/graph gate 364 passed.
- 2026-09-15 — PRD-143 #18d: `bayesian_transfer_acceptor_maps` -- the sensitised (after transfer) and directly excited acceptor maps with their rotational partners, a faithful port of ucfret's `s79.sens_maps_grid`/`direct_maps` (sampled exponentials, bin factor, non-periodic trapezoid convolution; `_conv` is exactly the causal convolution, so it runs on the existing `convolve_causal_ad`). The pool helpers `bayesian_thread_pool`/`bayesian_serial_here`/`bayesian_parallel_for` moved from `BayesianDecayModel.h` to the new `internal/BayesianParallel.h` (no copy; a module reconfigure picks up the new header); rate and acceptor maps run on it (acceptor maps 4.3 s, 4 threads). Against the Python CBM56 maps (ucfret `transfer_gate.cpp`): S_Ag 3.1e-10, S_Ag_rot_r 6.9e-12, A_dir 4.5e-12, A_dir_rot_r 3.1e-12 of their peaks; tau_a 0.1 % off fails. Tolerance 1e-9 from the measured floor (numpy LU vs Cholesky on the worst, cancelling map: 3.4e-10). Tests: efficiency total, zero without transfer, direct unit, partner area. ucfret's CBM56 gates unchanged after the pool move; 39 tests pass. Flagged for 18f: the sensitised partner applies e^{-t/rho} before and after the acceptor kernel, as the prototype does.
- 2026-09-15 — PRD-143 #18e: the loader builds the transfer tensors from a manifest `transfer` block (`bayesian_decay_build_transfer_tensors`; block and arrays together refused). CBM56 with C++-built tensors (ucfret `CBM56_TENSORS=cpp`): all B3-B7 gates pass (amplitudes 2.3e-8 under a 1e-6 coefficient tolerance, counts 2.7e-13, evidence 0.001 nats), the fit reaches the polished Python mode (p(R/R0) 6.4e-6, D/dof 1e-5). Building on load costs 4.4 s (run 6.4 s); the 3 s path keeps the tensors as arrays. Test `test_bayesian_decay_transfer_block.py`; 43 tests pass.
- 2026-09-15 — Python node types by name (tpeulen: "the cpp path must stay slim ... unimportant (no hotloop) stuff is allowed to be python via callbacks" / "important the callback"): `GraphNodeRegistry.register_python_type(type, factory)` and `IMP.bff.register_node_type(type[, factory])` (decorator form) in pyext/include/IMP_bff.core.i. A description names the type like a C++ one. The factory returns a GraphNode subclass; C++ receives a shared_ptr whose deleter owns a strong reference to the Python proxy, so the proxy lives exactly as long as the graph (the director alone holds it weakly, T-20260901-13). Factory errors cross as ValueError naming the type and the Python error; a non-node return is refused. Graph evaluation is single-threaded, so a Python node runs on the evaluating thread. Tests test/node/test_python_node_types.py: create by name, a description evaluates and fits (slope recovered to 1e-5) through a Python node after gc.collect(), the two refusals. Gate 751 passed.
- 2026-09-15 — PRD-143 A3 (tpeulen: "is there sth in boost to improve accuracy?" / "do it"). The ridge projection of the transfer maps is solved by Householder QR of `[B; sqrt(lambda) I]` instead of the normal equations. tttrlib `RidgeSolver::qr` (00acf2166, re-vendored) is now the default of `bayesian_transfer_projector`. Against a 50-digit solution on the CBM56 basis the coefficients go from 3.0e-7 to 2.1e-12 and the decays from 4.5e-12 to 1.6e-15. The Python side (`s79._ridge`, `MAP_SOLVER='ridgeqr'`, selected by `cbm56.environment(maps='exact')`) was switched too, with a new cache tag; the old maps stay reachable as `maps='exact_normal_equations'`. C++ against Python now agrees to about 1e-12 in coefficients and 5e-13 in decays across all maps. Boost.Multiprecision was not used: plain double precision with QR is enough. 44 decay tests pass, including the CBM56 regression on the re-emitted fixture.
- 2026-09-15 — t e^{-kt} components end to end (open point from the classic port): `TCSPCDecay` takes an optional `spectrum_kinds` input (one kind per component, non-zero = `a t e^{-t/tau}`), carried through amplitude compaction; such a component is convolved as minus the rate derivative of the same periodic kernel (`fconv_per_cs_ad` / `fconv_ad` on `tttrlib::Dual<double>`, tau seeded with d tau/dk = -tau^2), so there is no second convolution and the earlier pulses' `(t+nT) e^{-k(t+nT)}` come with it; unconvolved in closed form, basis columns likewise. `PhotophysicsTransferKineticsNode` with a `kinds` output emits a degeneracy as one flagged t e^{-kt} component (`internal::transfer_kinetics_spectrum_kinds`; without the output the eps split is unchanged); `PhotophysicsAnisotropySpectrumNode` and `LifetimeSpectrumMixture` carry kinds (`spectrum_kinds` / `k{i}` in, `kinds` out). tcspc_pddem wires transfer -> polarized -> decay. Tests: split-exponential limit 1e-6 at h = 1e-4, periodic closed form 1e-12, basis 1e-12, and at 1/tauA + k = 1/tauB the unconvolved periodic PDDEM curve equals the matrix exponential summed over pulses to 1e-10.
- 2026-09-15 — PDDEM alpha as reports: `presentation.reports` alpha_a / alpha_b call chisurf `chisurf.core.fluorescence.fret.pddem:emission_share` (mA/(mA+mB), as the classic group computed it); not a hot loop, so not a node.
- 2026-09-15 — VV/VH by group position as data: `presentation.group_position` (`scalar`, `single`, `alternate`) in the eight polarised TCSPC descriptions; ChiSurf's DescriptionModel sets the scalar for every view of the family in a fit group (one fit VM, then VV, VH by index), the classic rule.
- 2026-09-15 — Crosstalk ridge by QR (bayesian.decay af5d86b): `invert_block` factors with `RidgeSolver::qr`; 7e-14 from the Cholesky result on the fixture. Gate 773 passed.
- 2026-09-15 — PRD-143 A4.1 (tpeulen: "poisson score tttrlib - also used in fit2x"): the Poisson deviance, deviance residuals, score and information blocks moved verbatim to tttrlib `PoissonScore.h` (7456359a6; neutral names, test incl. `twoIstar = deviance/2N` at matched totals), vendored as `internal/PoissonScore.h` and pinned. `BayesianFisherScoring.h` keeps the soft floor only; `FitStatistics.h` forwards; the decay posterior calls `tttrlib::poisson_score_blocks`. ucfret CBM56 gate and fit output byte-identical; 43 tests pass.
- 2026-09-15 — PRD-143 A4.2 (tpeulen: "if fft is faster use that, but make sure produces same result"): the acceptor maps' trapezoid convolution by FFT (`internal::BayesianTrapezoidConvolver`, the response's and kernels' transforms kept, padded to L >= 2n as the prototype does). Equal to the direct causal sum to 2.3e-16 of peak (test), gate against the Python unchanged (S_Ag 4.6e-13); acceptor maps 4.7 s -> 1.24 s; CBM56 with tensors built on load 3.14 s end to end (was 7.2 s).
- 2026-09-15 — PRD-143 A4.3 (tpeulen: "convolution periodic"): exact acceptor maps (`BayesianAcceptorConstruction::exact`, `transfer.acceptor_maps: exact`) -- the one-way transfer kinetics' components (`transfer_pair_components_t`, exponential or t e^{-kt} at the degeneracy) through the exact periodic kernels (`BayesianPeriodicKernel` now per-column order). Against an independent brute force: 2e-14 / 2e-14 (degenerate) / 2e-13 (rotated); the prototype's construction 2.7e-2 off. On CBM56 the prototype's maps differ by 4 % (sensitised), 41 % (rotational partner: exp(-t/rho) applied twice) and 1.7 % (direct). Green fit with exact maps: evidence +0.38 nats, p(R/R0) 1.5e-3 of peak, Rule 0 unchanged. The double depolarisation factor is put to tpeulen.
- 2026-09-15 — PRD-143 A4.4 #16: the P-spline's B-spline basis in C++ (`bayesian_pspline_basis`, `BayesianPSpline.h`; loader builds `spl` from `pspline.basis_degree`). scipy `BSpline.design_matrix` to 1e-13, CBM56's matrix 1.7e-15; the tensor-free CBM56 fixture now carries neither tensors nor `spl` and passes every gate (3.35 s end to end). #24 (start values) waits for the MaxEnt engine another session is vendoring.
- 2026-09-15 — PRD-143 A4.5 (#13, tpeulen: "yes, unless too much perf hits"): the crosstalk mixing arithmetic once, `internal/CrosstalkMixing.h` (`crosstalk_mix`, templated); the Bayesian decay model's emission step runs through it. CBM56 fit output byte-identical, no measurable cost (medians 2.46 s / 2.34 s). The compiled `crosstalk_apply_mixing` switches onto it at the model-search session's next build.
- 2026-09-15 — MaxEnt TCSPC analysis moved into the engine (tpeulen: "maxent is better in bff than tttrlib, reloc" / "but you'll need to fix chisurf then"). tttrlib keeps the Skilling-Bryan engine, vendored as `internal/MaxEntQp.h` with the `Mat.h` it solves with (both pinned in test_vendored_headers; the std-only rule now admits a sibling that is itself vendored). New nodes: `SpectrumGrid` (from/to/bins inputs -- a sourceless node reports itself valid and is never pulled -- linear or log) and `MaxEntSpectrum` (bins x species basis from a TCSPCDecay, one amplitude per species or, grouping "fret", per distance over FRETSpectrumNode's layout; additive background + scatter on the prepared response; one common column scale so the amplitudes are fractions of the fluorescence counts, which is what a normalised prior regularises; `run_mem`, or `run_mem_target_chisq` for historic MaxEnt; outputs distribution, chisq, chisq_pearson (model-weighted, never optimised against), entropy, nu, converged). TCSPCDecay gains config `emit_basis` and output `prepared_response`. Families `tcspc_maxent_lifetime` (lifetime grid) and `tcspc_maxent_fret` (distance grid through the FRET rates, donor lifetimes axis): the grid's decays are the instrument's own basis, so there is no second convolution or IRF preparation (tttrlib's MaxEntTcspc carried copies of both). Known answers (test/decay/test_maxent_spectrum.py): lifetimes 1 and 4 ns at 0.5/0.5 recovered to 0.15 ns and 0.05 in mass; distances 40/65 Å at 0.6/0.4 to the grid point and 0.05; fitted curve = basis x solution + background + scatter r to 1e-9; target chi-square met to 2 %.
- 2026-09-15 — crosstalk_apply_mixing on internal/CrosstalkMixing.h's `crosstalk_mix` (bayesian.decay 2157e04, PRD-143 #13): the Eigen product goes; bit-identical on a 3x5x17 fixture. Gate 790 passed.
- 2026-09-15 — PRD-143 A4.3 finished on both sides (tpeulen on the prototype-vs-exact acceptor-map figure: "looks pretty bad, hope you fix it"). The exact periodic acceptor maps are the default in bff and in the Python prototype (`s79.ACCEPTOR_MAPS='exact'`, cache tag `_accexact`). C++ and Python agree to 1.6e-13, except near-degenerate rate pairs, which are gated with a tolerance scaled by the conditioning. The CBM56 fixtures were re-emitted on these maps and every B3-B7 gate passes. The fit from the fixed start does not reach the polished mode (mode 2 on one fixture, a ripple 0.05 nats below on the other): A4.7 (declared starts) is next.
- 2026-09-15 — PRD-143 A4.6/A4.7 measurements. The free-lambda prior is in `BayesianDecayPosterior.h` and tested (`test_bayesian_decay_free_lambda.py`). Its joint mode on CBM56 is degenerate: log10 lambda 3.94, 52.6 nats above the held node, because p(c|lambda) grows without bound near the penalty's null space. The evidence with c integrated out peaks at the grid's rough end: weight 0.40 at log10 lambda -1, 0.025 at the held value 1. So lambda is varied by the evidence-weighted grid, and a sampler for the joint density. Declared starts: on the exact maps every plain start ends in the second mode; an Adam phase of 150 steps before scoring reaches the polished mode. Drivers `cbm56_lambda_grid.cpp` and `cbm56_starts.cpp` (ucfret).
- 2026-09-15 — PRD-143 A4.6/A4.7 implemented: `bayesian_decay_declared_starts` / `bayesian_decay_fit_best_of_starts` (the data start and three Adam phases, concurrent, best kept, failed starts never win) and `bayesian_decay_fit_lambda_grid` (evidence-weighted mixture over log10 lambda). On CBM56: 3 of 4 starts reach the polished mode; the evidence favours log10 lambda around -1 (the held 1 has weight 0.017); the mixture mean R/R0 is 1.0787 +- 0.0305. p(R/R0)'s shape changes from two narrow peaks to one broad one across the grid at nearly constant D/dof. 19 s in all. Test `test_bayesian_decay_starts_and_lambda.py`; 61 decay tests pass.
- 2026-09-15 — PRD-143 A4.8 answered (tpeulen: "could the issue be laplace? RH110 only has single lifetime"). Neither the Laplace approximation nor the lifetime: a reproducible +-1 % channel nonlinearity of the gs detector. It replicates in the second Rh110 measurement (profiles correlate 0.80) and transfers as a `linearization` estimated on the other measurement (rhd110 z +4.40 -> -0.27, all histograms pass; rhd110_2 z +8.1 -> +3.9). A shifted control makes it worse. An independent linearization measurement is the proper source (ucfret figure s89_cbm56_dnl).
- 2026-09-15 — tcspc_polarized fits no rotation at magic angle: the rotation axis m runs from min(1,polarization) to 2*min(1,polarization) and r0 is freed once per rotation, so VM has only rotations.0 structures and r0 held (the classic lifetime model fixed rotations at VM; the view had been freeing an unidentifiable rho and r0). test/mcts/test_polarized_rotations.py.
- 2026-09-15 — PRD-143 A4.9: the 11.7-nat Fisher vs exact-Hessian evidence gap is the second-derivative term (-11.70; observed vs expected information only +0.04). It sits in weakly identified blocks: donor spectrum 5.3, acceptor weights 2.4, rotational weights, spline. New: `bayesian_decay_hessian` (differences of the analytic gradient), `bayesian_decay_laplace_evidence_exact` (equal to Python autograd to 1e-4 nats, 1.8 s) and `bayesian_decay_polish_exact`, test `test_bayesian_decay_hessian.py`. The exact evidence is unstable across the lambda grid, even after polishing (jumps of ~2 nats, indefinite at one node), so the Laplace approximation is not adequate there; the grid stays on Fisher by default, and the sampled posterior (A4.10) is needed. 64 decay tests pass.
- 2026-09-15 — PRD-143 A4.8 revised (tpeulen: "irf?"). The Rh110 VH wave depends on which water IRF is used: with h20_2 instead of h20, runs p goes 1.6e-4 -> 0.34 while the excess D/dof stays and VV worsens. A timing offset and IRF counting noise (six resampled IRFs) are excluded. So the wave is either a gs detector nonlinearity or a response mismatch (scatter IRF vs fluorescence colour); these data do not separate the two. A response-tail model or a colour-matched IRF would decide it (ucfret okf/log.md).
- 2026-09-15 — PRD-144 steps 1-2 (tpeulen: "irf?" then "2"): a fitted fluorescence-response tail per detector, `(1 - a) u + a u (*) k(tau)` in the lifetime columns (the scatter column keeps u), with analytic tangents (d k0/d ln tau = k1 - k0 for the bin-integrated periodic kernels) in `bayesian_response_basis` and the decay model's responses table and count Jacobian. Tests `test_bayesian_response_tail.py` and `test_bayesian_decay_response_tail_model.py`; CBM56 gates identical; 70 tests.
- 2026-09-15 — PRD-144 step 3: on CBM56 the fitted gs response tail goes to zero at every held time constant (0.1-3.2 ns) and leaves Rh110 VH unchanged (D/dof 1.356-1.359). The tail model is rejected; a detector nonlinearity or a non-broadening response difference remain.
- 2026-09-15 — PRD-145 step 1 (tpeulen: "go with sampling first, impl the convergence analysis"): MCMC convergence diagnostics once, in tttrlib (`McmcDiagnostics.h`: rank-normalised split R-hat bulk/tail, bulk/tail ESS with Geyer truncation, ESS of the mean, MCSE; Vehtari et al. 2021), vendored and pinned; equal to arviz to 1e-9 on six chain sets including non-mixing ones.
- 2026-09-15 — PRD-145 step 2 measured: the existing blocked Metropolis on CBM56 has tau ~13 000 steps (mode switching, ESS 400 per chain ~13 h); a gradient-based and/or mode-crossing sampler needs a PRD and tpeulen's decision.
- 2026-09-15 — PRD-145: the ensemble slice sampler ("slice", zeus's differential move) on CBM56 is slower still than the Metropolis per log posterior (tau >= 216 sweeps of 274 walkers, >400 000 evaluations per independent draw; walkers-as-chains R-hat 3.2). Neither existing sampler reaches the targets.
- 2026-09-15 — PRD-146 (tpeulen: "try NUTS."): the No-U-Turn sampler in tttrlib (`NoUTurnSampler.h`: Stan's multinomial NUTS, generalised U-turn, dense metric, dual averaging), tested on known targets and vendored. In bff: `BayesianDecaySampling.h` (NUTS chains on the decay posterior's gradient, the convergence analysis) and a gradient path without the information matrix. The synthetic experiment diverges where its response is clamped (a non-smooth posterior; the convergence test is marked as a strict expected failure). On CBM56 one chain has no divergences after warm-up and mixes mean R/R0 at tau 1.6 draws, but needs 451 gradients per draw at step 0.011, so ESS 400 on every summary would take hours; metric adaptation is the next step.
- 2026-09-17 — PRD-136: conditional dye-TITO training had not run since 09-11 (resume weights_only crash loop, attached.h5 rebuild stopped at the first pAcF run, PaiNN scatter row count bug); all three fixed, training resumed on 223 site runs. The anisotropy comparison now covers 39 sites incl. 10 T4L pAcF: RMSE 0.074, Spearman +0.08; pAcF sites anti-correlated (Pearson -0.69), 172L 5 and 8 worst.
- 2026-09-17 — PRD-150 (tpeulen: "CTBN amalgamation could be useful in bff"): first the survey. Product schemes occur in three places: chisurf's polarised smFRET simulator (conformation x emission mode), conformation x photophysics for FCS saturation, and FRETExchange's donor x acceptor label kinetics. Energy transfer (PhotophysicsTransferKinetics) does not factor, because D*A -> DA* changes two variables in one event. Then the port: `include/KineticNetwork.h` has `KineticIntensityMatrix` (CIM, extract, aGrUM's amalgamate), `KineticNetwork` (CTBN), `kinetic_stationary_distribution` and `kinetic_transient_distribution` (Eigen expm). Matrices are `K[target, source]`, the joint index is C-order over declaration order, and the diagonal is derived. aGrUM's `Q[from, to]` is little-endian over sorted names, and `K = Q[perm][:, perm].T`. Tests in `test/kinetics/test_kinetic_network.py` (12): the PET FCS 6-state dark/exc matrices and the simulator's k_nrad/k_rad (tttrlib layout) equal their hand-written forms exactly; fcs_bunching_factor is identical from either; A/B against a pyagrum-free transcription of aGrUM (`test/kinetics/agrum_ctbn_reference.py`, fixture `test/kinetics/data/ctbn_agrum_fixture.json`) gives generators to 8.9e-16 and SimpleInference posteriors to 1.6e-15. Open: adopt it in simulate.py; the FRETExchange pair composes transition matrices (Kronecker product, an extra dt M_d (x) M_a term) rather than generators.
- 2026-09-17 — PRD-151 (tpeulen: "agrum is bff domain, the algos."): aGrUM's linear-Gaussian inference in C++. `include/InferenceCanonicalForm.h`: Gaussian factors `(K, h, g)` over a named, sized scope with aGrUM's product/divide/extend/reduce/marginalize/fromCLG and ChiSurf's from_moments, log normaliser, strict condition, ordered marginal; a singular K is carried (rank, null space) and only the moments refuse it. `include/InferenceGaussianElimination.h`: aGrUM's `canonicalPosterior` over an `InferenceFactorGraph`, plus exact relevance pruning by evidence separation (Bayes-ball rejected: barren nodes need normalised conditionals). `InferenceFactorGraph::get_elimination_order("weighted")`: aGrUM's default triangulation with sizes as log domain sizes (its quasi-simplicial phase is unreachable, an integer division). A/B against the pyagrum-free transcription `test/factorgraph/agrum_clg_reference.py` (fixture `clg_agrum_fixture.json`): algebra 2.2e-16, posteriors 2.5e-14, orders equal; against the brute-force joint: means 3.6e-14, covariances 2.3e-13, log evidence 1.2e-14 (`test_inference_gaussian.py`, 41). ChiSurf's `canonical.py` is deleted; `GaussianEngine` holds the bff form and `LaplaceEngine.condition` is a certified closed form (re-fit only when the certificate fails). Discrete BN/MRF core, structure learning, causal, explain re-evaluated for bff and rejected with reasons in the PRD; one candidate recorded (exact joint for multi-label rotamer weights). junk/aGrUM has nothing further to harvest.
- 2026-09-18 — The workshop's labelizer step runs with conservation on (`03_labelizer.ipynb` + `03b_labelizer_pairs.ipynb`): hGBP1 has real ConSurf grades after all. Where they are not: labelizer.org holds none for 1F5N (`conservationscore/229` → no chains; an analysis job with cs weighted, `02daaad8-…`, terminated with LS/CR/SE/SS tables and no cs — the pipeline's empty answer now measured on 3GUN, 1DDB and 1F5N). Where they are: ConSurf-DB, unreachable at the 2026-09-17 check, answers today over a REST surface of its own (`find_chains` → `get_final_data` → the `.grades` table; the B-factor PDB URL it also lists 404s): a finished 1F5N chain A run, 6252 homologues, 300 in the final MSA. `workshop.py` gains `hgbp1_consurf_grades()`: the webserver is asked first — the `ipynb/example` contract, `load_pdb` → `conservationscore` — so the day its internal ConSurf job delivers, that route wins; ConSurf-DB is the source that answers today. The grades arrive on 1F5N numbering (modelled 7-583, disordered 158-164, = 570 residues) and the CG topology numbers its own 570 1-570, so the k-th topology residue takes the k-th modelled position's grade — identity-verified 570/570 before anything is written, and the crystal's gap is exactly where the topology's A|B label boundary falls. The artifact is one atom per residue of the topology, the normalised grade in the B-factor (the `.grades` ATOM column is `MET:7:A`, the new ConSurf-DB shape `labelizer_read_consurf` does not parse — hence the PDB form), cached in `_data/`. Notebook 03 keeps the no-grades zero-veto demo, then runs the full paper model; a with/without scatter follows (cmap `RdYlGn`, red conserved below the diagonal — `_r` reverses ConSurf's own convention; grade-vs-shift correlation +0.76, top-10 overlap 6/10, cs demotes B270/B312/B489/B200); conservation joins TERMS and `used`, the geometric-mean check now four terms and still exact; 26 candidates, B199 leads at 1.97. 03b follows on the same model — otherwise the two notebooks would disagree on the shortlist — and its measured prose is reconciled to the new run: 69 sites at 1.5 → 2346 one-state pairs, 53 both-state sites → 1378 two-state pairs in 10 s; the one-state winners (B374-B457, A151-B199, B199-B270) sit on sites that stop being labelable (B270 1.70 → 1.04 across the transition); the site audit now finds **no dead site and one thin one** — B383, the list's third-best label score at 1.97, 1628 → 309 grid points — so the dead-site lesson keeps the mechanism and the hedge, with B383 the live example; and the cone's number 108 becomes the rebuild's number 1. One portability bug the re-run caught: `bff.labelizer_read_structure(...).residues` on a temporary returns an empty container — hold the structure object. One-off `_add_consurf_ws.py` beside the workshop notebooks, after `ipynb/example/_add_consurf_nb.py`.
- 2026-09-18 — The workshop's "score on the fold" panels actually colour by score (tpeulen: "the colors are not mapped to the 3d structure"). Three defects stacked in the Mol* wiring of `viewer_html`, reproduced in a browser and fixed: (1) the `theme` was passed to the trajectory hierarchy preset, which has no such param — it belongs inside `representationPresetParams`, the representation preset's CommonParams, so it was silently dropped and the cartoon fell back to chain-id colours; (2) the "uncertainty" theme does not scale itself to the data (`domain` defaults to [0, 100], the B-factor scale — a TODO in Mol* itself), so the score ramp now takes its domain from the B-factor column of the score panels, one domain shared by every score panel so two frames colour on the same scale; (3) Mol*'s `ColorScale` interpolates its list as Color *numbers* — hex strings (which the param system parses before the UI ever sees them) interpolate to NaN and render black — and the uncertainty theme builds its scale `reverse: true`, so the list is `[0xb2182b, 0xf7f7f7, 0x2166ac]`: red listed first lands on the domain's good end, blue on the poor one. Verified in-browser on the executed notebook's own HTML output: the fold runs blue core to red exposed tips in both frames. `03_labelizer.ipynb` re-executed (0 errors); it is the only `colour_by="score"` user, and every other panel's behaviour is unchanged.
- 2026-09-18 — PRD-136: conditional dye surrogate sampled across all 30 representative sites (T4L: 6, MalE apo: 8, MalE holo: 8, hGBP1: 8) on heinzehub GPU 0 (`s14_sample_all_sites.py`, `cond_v4` best ckpt at 39k steps; 24 chains x 60 steps at 100 ps lag = 144 ns/site, 4.3 µs aggregate model time). Completed cleanly in ~11 h. Results synced (`data/s14_model_all.npz`, `figures/s14_sample_model_all.txt`): mean spatial density overlap with MD is 0.370 ± 0.162 across 28 in-sample sites (range 0.025 to 0.678), with held-out sites at 0.222 (T4L 172L_132) and 0.354 (hGBP1 1F5N_481), matching the ~0.45 MD half-vs-half control and far exceeding the 0.01–0.15 MD restart-to-restart overlap; contact error |d_contact| averages 0.116 (median 0.056, 22/30 sites < 0.10); chirality repair keeps 84.5 ± 8.3 %; rotational relaxation fails (mean max|dC| 0.739, held-out ~0.94). Confirms across all three target proteins that the surrogate captures the spatial density and surface contacts, while surface rotational dynamics are not captured.
