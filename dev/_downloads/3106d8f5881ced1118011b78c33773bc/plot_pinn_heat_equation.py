...
``IMP.bff.NeuralNet`` provides both without an autodiff framework:
...
SciPy's L-BFGS-B works on the flat ``get_parameters()`` vector. This is the same
...
def make_net(dims, activation="tanh", seed=0):
    ...
    ``train_neural_net`` fits the scalers itself; a network that will be
    trained on a residual is built explicitly, as a ``bff.neural_net``
    msgpack document.
    """
    ...
    return IMP.bff.NeuralNet(msgpack.packb(
        {"format": "bff.neural_net", "version": 1, "layers": layers}, use_bin_type=True))
...
def objective(p):
    ...
    _, _, u_xx = net.predict_derivatives(X_f, e_x, order=2)
    _, u_t, _ = net.predict_derivatives(X_f, e_t, order=1)
    u_d, _, _ = net.predict_derivatives(X_d, order=0)
    r = u_t - alpha * u_xx
    loss = np.mean(r ** 2) + np.mean((u_d - U_d) ** 2)
    g_xx, _, _ = net.backward_derivatives(X_f, e_x, np.zeros_like(r), dY2=-alpha * 2 * r / r.size)
    g_t, _, _ = net.backward_derivatives(X_f, e_t, np.zeros_like(r), dY1=2 * r / r.size)
    g_d, _ = net.backward(X_d, 2 * (u_d - U_d) / U_d.size)
    history.append(loss)
    return loss, g_xx + g_t + g_d


res = minimize(objective, net.get_parameters(), jac=True, method="L-BFGS-B",
...
u_net = predict(net, XT).reshape(xg.shape)
...
for t in (0.0, 0.25, 0.5, 1.0):
    ...
    ax[0].plot(xs, predict(net, XTs), "--", label="t = %.2f" % t)
    _, _, uxx = net.predict_derivatives(XTs, np.tile([[1.0, 0.0]], (len(xs), 1)), order=2)
    _, ut, _ = net.predict_derivatives(XTs, np.tile([[0.0, 1.0]], (len(xs), 1)), order=1)
